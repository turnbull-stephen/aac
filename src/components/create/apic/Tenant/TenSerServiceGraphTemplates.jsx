...
import TenSerServiceGraphTemplatesDevice from './TenSerServiceGraphTemplatesDevice'
import TenSerServiceGraphTemplatesConsumerProvider from './TenSerServiceGraphTemplatesConsumerProvider'
import TenSerServiceGraphTemplatesExpectedState from './TenSerServiceGraphTemplatesExpectedState'


const TenSerServiceGraphTemplates = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Service Graph Templates'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerServiceGraphTemplatesDeviceActive = activeKeys[item.id] === 'tenserservicegraphtemplatesdevice';
          const isTenSerServiceGraphTemplatesConsumerProviderActive = activeKeys[item.id] === 'tenserservicegraphtemplatesconsumerprovider';
          const isTenSerServiceGraphTemplatesExpectedStateActive = activeKeys[item.id] === 'tenserservicegraphtemplatesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserservicegraphtemplates'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenserservicegraphtemplates" title="Service Graph Templates">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_service_graph_templates', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserservicegraphtemplatesdevice" title="Device">
                        {isTenSerServiceGraphTemplatesDeviceActive && (  
                            <TenSerServiceGraphTemplatesDevice
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].device`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserservicegraphtemplatesconsumerprovider" title="Consumer">
                        {isTenSerServiceGraphTemplatesConsumerProviderActive && (  
                            <TenSerServiceGraphTemplatesConsumerProvider
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenserservicegraphtemplatesconsumerprovider" title="Provider">
                        {isTenSerServiceGraphTemplatesConsumerProviderActive && (  
                            <TenSerServiceGraphTemplatesConsumerProvider
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenserservicegraphtemplatesexpectedstate" title="Expected State">
                        {isTenSerServiceGraphTemplatesExpectedStateActive && (  
                            <TenSerServiceGraphTemplatesExpectedState
                              key={item.id}
                              control={control}
...
export default TenSerServiceGraphTemplates;

...
import TenSrMplsL3outsExternalEndpointGroupsSubnets from './TenSrMplsL3outsExternalEndpointGroupsSubnets'
import TenSrMplsL3outsExternalEndpointGroupsContracts from './TenSrMplsL3outsExternalEndpointGroupsContracts'


const TenSrMplsL3outsExternalEndpointGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSrMplsL3outsExternalEndpointGroupsSubnetsActive = activeKeys[item.id] === 'tensrmplsl3outsexternalendpointgroupssubnets';
          const isTenSrMplsL3outsExternalEndpointGroupsContractsActive = activeKeys[item.id] === 'tensrmplsl3outsexternalendpointgroupscontracts';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tensrmplsl3outsexternalendpointgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tensrmplsl3outsexternalendpointgroups" title="External Endpoint Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_sr_mpls_l3outs_external_endpoint_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tensrmplsl3outsexternalendpointgroupssubnets" title="Subnets">
                        {isTenSrMplsL3outsExternalEndpointGroupsSubnetsActive && (  
                            <TenSrMplsL3outsExternalEndpointGroupsSubnets
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tensrmplsl3outsexternalendpointgroupscontracts" title="Contracts">
                        {isTenSrMplsL3outsExternalEndpointGroupsContractsActive && (  
                            <TenSrMplsL3outsExternalEndpointGroupsContracts
                              key={item.id}
                              control={control}
...
export default TenSrMplsL3outsExternalEndpointGroups;

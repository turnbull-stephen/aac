...
import TenMplsCustomQosIngressRules from './TenMplsCustomQosIngressRules'
import TenMplsCustomQosEgressRuless from './TenMplsCustomQosEgressRuless'


const TenMplsCustomQosPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Mpls Custom Qos Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenMplsCustomQosIngressRulesActive = activeKeys[item.id] === 'tenmplscustomqosingressrules';
          const isTenMplsCustomQosEgressRulessActive = activeKeys[item.id] === 'tenmplscustomqosegressruless';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenmplscustomqospolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenmplscustomqospolicies" title="Mpls Custom Qos Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_mpls_custom_qos_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenmplscustomqosingressrules" title="Ingress Rules">
                        {isTenMplsCustomQosIngressRulesActive && (  
                            <TenMplsCustomQosIngressRules
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].ingress_rules`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenmplscustomqosegressruless" title="Egress Rules">
                        {isTenMplsCustomQosEgressRulessActive && (  
                            <TenMplsCustomQosEgressRuless
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].egress_rules`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenMplsCustomQosPolicies;

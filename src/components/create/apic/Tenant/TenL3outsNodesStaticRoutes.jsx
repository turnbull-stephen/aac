...
import TenL3outsNodesStaticRoutesNextHops from './TenL3outsNodesStaticRoutesNextHops'


const TenL3outsNodesStaticRoutes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodesStaticRoutesNextHopsActive = activeKeys[item.id] === 'tenl3outsnodesstaticroutesnexthops';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodesstaticroutes'}
                    ...
                    >
                      <Tab eventKey="tenl3outsnodesstaticroutes" title="Static Routes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_nodes_static_routes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsnodesstaticroutesnexthops" title="Next Hops">
                        {isTenL3outsNodesStaticRoutesNextHopsActive && (  
                            <TenL3outsNodesStaticRoutesNextHops
                              key={item.id}
                              control={control}
...
export default TenL3outsNodesStaticRoutes;

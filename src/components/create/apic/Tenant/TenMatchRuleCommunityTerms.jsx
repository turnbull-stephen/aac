...
import TenMatchRuleCommunityTermsFactors from './TenMatchRuleCommunityTermsFactors'


const TenMatchRuleCommunityTerms = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenMatchRuleCommunityTermsFactorsActive = activeKeys[item.id] === 'tenmatchrulecommunitytermsfactors';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenmatchrulecommunityterms'}
                    ...
                    >
                      <Tab eventKey="tenmatchrulecommunityterms" title="Community Terms">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_match_rule_community_terms', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenmatchrulecommunitytermsfactors" title="Factors">
                        {isTenMatchRuleCommunityTermsFactorsActive && (  
                            <TenMatchRuleCommunityTermsFactors
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].factors`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenMatchRuleCommunityTerms;

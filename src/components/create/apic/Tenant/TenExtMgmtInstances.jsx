...
import TenExtMgmtInstancesOobContracts from './TenExtMgmtInstancesOobContracts'
import TenExtMgmtInstancesExpectedState from './TenExtMgmtInstancesExpectedState'


const TenExtMgmtInstances = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Ext Mgmt Instances'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenExtMgmtInstancesOobContractsActive = activeKeys[item.id] === 'tenextmgmtinstancesoobcontracts';
          const isTenExtMgmtInstancesExpectedStateActive = activeKeys[item.id] === 'tenextmgmtinstancesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenextmgmtinstances'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenextmgmtinstances" title="Ext Mgmt Instances">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ext_mgmt_instances', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenextmgmtinstancesoobcontracts" title="Oob Contracts">
                        {isTenExtMgmtInstancesOobContractsActive && (  
                            <TenExtMgmtInstancesOobContracts
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenextmgmtinstancesexpectedstate" title="Expected State">
                        {isTenExtMgmtInstancesExpectedStateActive && (  
                            <TenExtMgmtInstancesExpectedState
                              key={item.id}
                              control={control}
...
export default TenExtMgmtInstances;

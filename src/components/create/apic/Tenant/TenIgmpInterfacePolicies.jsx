...
import TenIgmpInterfacePoliciesExpectedState from './TenIgmpInterfacePoliciesExpectedState'


const TenIgmpInterfacePolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Igmp Interface Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenIgmpInterfacePoliciesExpectedStateActive = activeKeys[item.id] === 'tenigmpinterfacepoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenigmpinterfacepolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenigmpinterfacepolicies" title="Igmp Interface Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_igmp_interface_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenigmpinterfacepoliciesexpectedstate" title="Expected State">
                        {isTenIgmpInterfacePoliciesExpectedStateActive && (  
                            <TenIgmpInterfacePoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenIgmpInterfacePolicies;

...
import TenSrMplsL3outsNodeProfiles from './TenSrMplsL3outsNodeProfiles'
import TenSrMplsL3outsExternalEndpointGroups from './TenSrMplsL3outsExternalEndpointGroups'


const TenSrMplsL3outs = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSrMplsL3outsNodeProfilesActive = activeKeys[item.id] === 'tensrmplsl3outsnodeprofiles';
          const isTenSrMplsL3outsExternalEndpointGroupsActive = activeKeys[item.id] === 'tensrmplsl3outsexternalendpointgroups';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tensrmplsl3outs'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tensrmplsl3outs" title="Sr Mpls L3outs">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_sr_mpls_l3outs', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isTenSrMplsL3outsNodeProfilesActive && (  
                            <TenSrMplsL3outsNodeProfiles
                              key={item.id}
                              control={control}
                        ...
                        {isTenSrMplsL3outsExternalEndpointGroupsActive && (  
                            <TenSrMplsL3outsExternalEndpointGroups
                              key={item.id}
                              control={control}
...
export default TenSrMplsL3outs;

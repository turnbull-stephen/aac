...
import TenSerRedirectPoliciesL1l2Destinations from './TenSerRedirectPoliciesL1l2Destinations'
import TenSerRedirectPoliciesL3Destinations from './TenSerRedirectPoliciesL3Destinations'
import TenSerRedirectPoliciesExpectedState from './TenSerRedirectPoliciesExpectedState'


const TenSerRedirectPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerRedirectPoliciesL1l2DestinationsActive = activeKeys[item.id] === 'tenserredirectpoliciesl1l2destinations';
          const isTenSerRedirectPoliciesL3DestinationsActive = activeKeys[item.id] === 'tenserredirectpoliciesl3destinations';
          const isTenSerRedirectPoliciesExpectedStateActive = activeKeys[item.id] === 'tenserredirectpoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserredirectpolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenserredirectpolicies" title="Redirect Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_redirect_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserredirectpoliciesl1l2destinations" title="L1l2 Destinations">
                        {isTenSerRedirectPoliciesL1l2DestinationsActive && (  
                            <TenSerRedirectPoliciesL1l2Destinations
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].l1l2_destinations`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserredirectpoliciesl3destinations" title="L3 Destinations">
                        {isTenSerRedirectPoliciesL3DestinationsActive && (  
                            <TenSerRedirectPoliciesL3Destinations
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].l3_destinations`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserredirectpoliciesexpectedstate" title="Expected State">
                        {isTenSerRedirectPoliciesExpectedStateActive && (  
                            <TenSerRedirectPoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenSerRedirectPolicies;

...
const  TenSerDeviceSelectionPoliciesConsumerProviderRedirectPolicy = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors, formData }) => {
   
  const [key, setKey] = useState('tenserdeviceselectionpoliciesconsumerproviderredirectpolicy'); // Keep state for the Tab selection
  ...
  const isTenSerDeviceSelectionPoliciesConsumerProviderRedirectPolicyActive = key === 'tenserdeviceselectionpoliciesconsumerproviderredirectpolicy';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        <Tab eventKey="tenserdeviceselectionpoliciesconsumerproviderredirectpolicy" title={tabTitle('Redirect Policy', hasTenSerDeviceSelectionPoliciesConsumerProviderRedirectPolicyErrors)}>
          {isTenSerDeviceSelectionPoliciesConsumerProviderRedirectPolicyActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'tenant.ten_ser_device_selection_policies_consumer_provider_redirect_policy', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default TenSerDeviceSelectionPoliciesConsumerProviderRedirectPolicy;

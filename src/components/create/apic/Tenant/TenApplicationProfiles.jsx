...
import TenApEndpointGroups from './TenApEndpointGroups'
import TenApEndpointSecurityGroups from './TenApEndpointSecurityGroups'
import TenApExpectedState from './TenApExpectedState'


const TenApplicationProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Application Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenApEndpointGroupsActive = activeKeys[item.id] === 'tenapendpointgroups';
          const isTenApEndpointSecurityGroupsActive = activeKeys[item.id] === 'tenapendpointsecuritygroups';
          const isTenApExpectedStateActive = activeKeys[item.id] === 'tenapexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenapplicationprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenapplicationprofiles" title="Application Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_application_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isTenApEndpointGroupsActive && (  
                            <TenApEndpointGroups
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].endpoint_groups`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        {isTenApEndpointSecurityGroupsActive && (  
                            <TenApEndpointSecurityGroups
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].endpoint_security_groups`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenapexpectedstate" title="Expected State">
                        {isTenApExpectedStateActive && (  
                            <TenApExpectedState
                              key={item.id}
                              control={control}
...
export default TenApplicationProfiles;

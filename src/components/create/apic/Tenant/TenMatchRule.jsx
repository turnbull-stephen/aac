...
import TenMatchRuleRegexCommunityTerms from './TenMatchRuleRegexCommunityTerms'
import TenMatchRuleCommunityTerms from './TenMatchRuleCommunityTerms'
import TenMatchRulePrefixes from './TenMatchRulePrefixes'


const TenMatchRule = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenMatchRuleRegexCommunityTermsActive = activeKeys[item.id] === 'tenmatchruleregexcommunityterms';
          const isTenMatchRuleCommunityTermsActive = activeKeys[item.id] === 'tenmatchrulecommunityterms';
          const isTenMatchRulePrefixesActive = activeKeys[item.id] === 'tenmatchruleprefixes';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenmatchrule'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenmatchrule" title="Match Rules">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_match_rule', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenmatchruleregexcommunityterms" title="Regex Community Terms">
                        {isTenMatchRuleRegexCommunityTermsActive && (  
                            <TenMatchRuleRegexCommunityTerms
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].regex_community_terms`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenmatchrulecommunityterms" title="Community Terms">
                        {isTenMatchRuleCommunityTermsActive && (  
                            <TenMatchRuleCommunityTerms
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].community_terms`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenmatchruleprefixes" title="Prefixes">
                        {isTenMatchRulePrefixesActive && (  
                            <TenMatchRulePrefixes
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].prefixes`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

...
import TenNdRaPrefixPoliciesExpectedState from './TenNdRaPrefixPoliciesExpectedState'


const TenNdRaPrefixPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Nd Ra Prefix Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenNdRaPrefixPoliciesExpectedStateActive = activeKeys[item.id] === 'tenndraprefixpoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenndraprefixpolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenndraprefixpolicies" title="Nd Ra Prefix Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_nd_ra_prefix_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenndraprefixpoliciesexpectedstate" title="Expected State">
                        {isTenNdRaPrefixPoliciesExpectedStateActive && (  
                            <TenNdRaPrefixPoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenNdRaPrefixPolicies;

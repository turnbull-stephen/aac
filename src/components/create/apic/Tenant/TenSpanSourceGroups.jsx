...
import TenSpanSourceGroupsSources from './TenSpanSourceGroupsSources'
import TenSpanSourceGroupsExpectedState from './TenSpanSourceGroupsExpectedState'


const TenSpanSourceGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSpanSourceGroupsSourcesActive = activeKeys[item.id] === 'tenspansourcegroupssources';
          const isTenSpanSourceGroupsExpectedStateActive = activeKeys[item.id] === 'tenspansourcegroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenspansourcegroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenspansourcegroups" title="Source Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_span_source_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenspansourcegroupssources" title="Sources">
                        {isTenSpanSourceGroupsSourcesActive && (  
                            <TenSpanSourceGroupsSources
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenspansourcegroupsexpectedstate" title="Expected State">
                        {isTenSpanSourceGroupsExpectedStateActive && (  
                            <TenSpanSourceGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default TenSpanSourceGroups;

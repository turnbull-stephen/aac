...
import TenIpSlaPoliciesExpectedState from './TenIpSlaPoliciesExpectedState'


const TenIpSlaPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenIpSlaPoliciesExpectedStateActive = activeKeys[item.id] === 'tenipslapoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenipslapolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenipslapolicies" title="Ip Sla Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ip_sla_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenipslapoliciesexpectedstate" title="Expected State">
                        {isTenIpSlaPoliciesExpectedStateActive && (  
                            <TenIpSlaPoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenIpSlaPolicies;

...
import TenVrfLeakedInternalPrefixesDestinations from './TenVrfLeakedInternalPrefixesDestinations'


const TenVrfLeakedInternalPrefixes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Leaked Internal Prefixes'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenVrfLeakedInternalPrefixesDestinationsActive = activeKeys[item.id] === 'tenvrfleakedinternalprefixesdestinations';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenvrfleakedinternalprefixes'}
                    ...
                    >
                      <Tab eventKey="tenvrfleakedinternalprefixes" title="Leaked Internal Prefixes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_vrf_leaked_internal_prefixes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenvrfleakedinternalprefixesdestinations" title="Destinations">
                        {isTenVrfLeakedInternalPrefixesDestinationsActive && (  
                            <TenVrfLeakedInternalPrefixesDestinations
                              key={item.id}
                              control={control}
...
export default TenVrfLeakedInternalPrefixes;

...
const TenVrfPimIgmpContextSsmTranslatePolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Igmp Context Ssm Translate Policies'
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenvrfpimigmpcontextssmtranslatepolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenvrfpimigmpcontextssmtranslatepolicies" title="Igmp Context Ssm Translate Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_vrf_pim_igmp_context_ssm_translate_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
...
export default TenVrfPimIgmpContextSsmTranslatePolicies;

...
import TenRouteControlRouteMapsContexts from './TenRouteControlRouteMapsContexts'


const TenRouteControlRouteMaps = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Route Control Route Maps'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenRouteControlRouteMapsContextsActive = activeKeys[item.id] === 'tenroutecontrolroutemapscontexts';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenroutecontrolroutemaps'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenroutecontrolroutemaps" title="Route Control Route Maps">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_route_control_route_maps', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenroutecontrolroutemapscontexts" title="Contexts">
                        {isTenRouteControlRouteMapsContextsActive && (  
                            <TenRouteControlRouteMapsContexts
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].contexts`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenRouteControlRouteMaps;

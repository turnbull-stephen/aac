...
import TenDhcpOptions from './TenDhcpOptions'


const TenDhcpOptionPolicy = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Dhcp Option Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenDhcpOptionsActive = activeKeys[item.id] === 'tendhcpoptions';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tendhcpoptionpolicy'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tendhcpoptionpolicy" title="Dhcp Option Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_dhcp_option_policy', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tendhcpoptions" title="Options">
                        {isTenDhcpOptionsActive && (  
                            <TenDhcpOptions
                              key={item.id}
                              control={control}
...
export default TenDhcpOptionPolicy;

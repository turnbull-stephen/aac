...
import TenSetRuleDampening from './TenSetRuleDampening'
import TenSetRuleAdditionalCommunities from './TenSetRuleAdditionalCommunities'
import TenSetRuleSetAsPath from './TenSetRuleSetAsPath'
import TenSetRuleExternalEndpointGroup from './TenSetRuleExternalEndpointGroup'


const TenSetRule = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSetRuleDampeningActive = activeKeys[item.id] === 'tensetruledampening';
          const isTenSetRuleAdditionalCommunitiesActive = activeKeys[item.id] === 'tensetruleadditionalcommunities';
          const isTenSetRuleSetAsPathActive = activeKeys[item.id] === 'tensetrulesetaspath';
          const isTenSetRuleExternalEndpointGroupActive = activeKeys[item.id] === 'tensetruleexternalendpointgroup';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tensetrule'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tensetrule" title="Set Rules">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_set_rule', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tensetruledampening" title="Dampening">
                        {isTenSetRuleDampeningActive && (  
                            <TenSetRuleDampening
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].dampening`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tensetruleadditionalcommunities" title="Additional Communities">
                        {isTenSetRuleAdditionalCommunitiesActive && (  
                            <TenSetRuleAdditionalCommunities
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].additional_communities`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tensetrulesetaspath" title="Set As Path">
                        {isTenSetRuleSetAsPathActive && (  
                            <TenSetRuleSetAsPath
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].set_as_path`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tensetruleexternalendpointgroup" title="External Endpoint Group">
                        {isTenSetRuleExternalEndpointGroupActive && (  
                            <TenSetRuleExternalEndpointGroup
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].external_endpoint_group`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

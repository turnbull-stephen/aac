...
import TenSrMplsL3outsNodeProfilesInterfaceProfilesInterfacesBgpPeers from './TenSrMplsL3outsNodeProfilesInterfaceProfilesInterfacesBgpPeers'


const TenSrMplsL3outsNodeProfilesInterfaceProfilesInterfaces = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSrMplsL3outsNodeProfilesInterfaceProfilesInterfacesBgpPeersActive = activeKeys[item.id] === 'tensrmplsl3outsnodeprofilesinterfaceprofilesinterfacesbgppeers';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tensrmplsl3outsnodeprofilesinterfaceprofilesinterfaces'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tensrmplsl3outsnodeprofilesinterfaceprofilesinterfaces" title="Interfaces">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_sr_mpls_l3outs_node_profiles_interface_profiles_interfaces', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tensrmplsl3outsnodeprofilesinterfaceprofilesinterfacesbgppeers" title="Bgp Peers">
                        {isTenSrMplsL3outsNodeProfilesInterfaceProfilesInterfacesBgpPeersActive && (  
                            <TenSrMplsL3outsNodeProfilesInterfaceProfilesInterfacesBgpPeers
                              key={item.id}
                              control={control}
...
export default TenSrMplsL3outsNodeProfilesInterfaceProfilesInterfaces;

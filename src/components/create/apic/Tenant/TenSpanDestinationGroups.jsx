...
import TenSpanDestinationGroupsExpectedState from './TenSpanDestinationGroupsExpectedState'


const TenSpanDestinationGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSpanDestinationGroupsExpectedStateActive = activeKeys[item.id] === 'tenspandestinationgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenspandestinationgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenspandestinationgroups" title="Destination Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_span_destination_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenspandestinationgroupsexpectedstate" title="Expected State">
                        {isTenSpanDestinationGroupsExpectedStateActive && (  
                            <TenSpanDestinationGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default TenSpanDestinationGroups;

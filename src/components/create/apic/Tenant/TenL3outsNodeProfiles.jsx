...
import TenL3outsNodeProfilesBgpPeers from './TenL3outsNodeProfilesBgpPeers'
import TenL3outsNodeProfilesNodes from './TenL3outsNodeProfilesNodes'
import TenL3outsNodeProfilesInterfaceProfiles from './TenL3outsNodeProfilesInterfaceProfiles'


const TenL3outsNodeProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodeProfilesBgpPeersActive = activeKeys[item.id] === 'tenl3outsnodeprofilesbgppeers';
          const isTenL3outsNodeProfilesNodesActive = activeKeys[item.id] === 'tenl3outsnodeprofilesnodes';
          const isTenL3outsNodeProfilesInterfaceProfilesActive = activeKeys[item.id] === 'tenl3outsnodeprofilesinterfaceprofiles';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodeprofiles'}
                    ...
                    >
                      <Tab eventKey="tenl3outsnodeprofiles" title="Node Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_node_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsnodeprofilesbgppeers" title="Bgp Peers">
                        {isTenL3outsNodeProfilesBgpPeersActive && (  
                            <TenL3outsNodeProfilesBgpPeers
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenl3outsnodeprofilesnodes" title="Nodes">
                        {isTenL3outsNodeProfilesNodesActive && (  
                            <TenL3outsNodeProfilesNodes
                              key={item.id}
                              control={control}
                        ...
                        {isTenL3outsNodeProfilesInterfaceProfilesActive && (  
                            <TenL3outsNodeProfilesInterfaceProfiles
                              key={item.id}
                              control={control}
...
export default TenL3outsNodeProfiles;

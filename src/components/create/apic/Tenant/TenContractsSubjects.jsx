...
import TenContractsSubjectsFilters from './TenContractsSubjectsFilters'


const TenContractsSubjects = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenContractsSubjectsFiltersActive = activeKeys[item.id] === 'tencontractssubjectsfilters';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tencontractssubjects'}
                    ...
                    >
                      <Tab eventKey="tencontractssubjects" title="Subjects">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_contracts_subjects', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tencontractssubjectsfilters" title="Filters">
                        {isTenContractsSubjectsFiltersActive && (  
                            <TenContractsSubjectsFilters
                              key={item.id}
                              control={control}
...
export default TenContractsSubjects;

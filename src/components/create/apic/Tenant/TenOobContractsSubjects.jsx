...
import TenOobContractsSubjectsFilters from './TenOobContractsSubjectsFilters'


const TenOobContractsSubjects = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenOobContractsSubjectsFiltersActive = activeKeys[item.id] === 'tenoobcontractssubjectsfilters';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenoobcontractssubjects'}
                    ...
                    >
                      <Tab eventKey="tenoobcontractssubjects" title="Subjects">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_oob_contracts_subjects', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenoobcontractssubjectsfilters" title="Filters">
                        {isTenOobContractsSubjectsFiltersActive && (  
                            <TenOobContractsSubjectsFilters
                              key={item.id}
                              control={control}
...
export default TenOobContractsSubjects;

...
import TenOobEndpointGroupsOobContracts from './TenOobEndpointGroupsOobContracts'
import TenOobEndpointGroupsExpectedState from './TenOobEndpointGroupsExpectedState'


const TenOobEndpointGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Oob Endpoint Groups'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenOobEndpointGroupsOobContractsActive = activeKeys[item.id] === 'tenoobendpointgroupsoobcontracts';
          const isTenOobEndpointGroupsExpectedStateActive = activeKeys[item.id] === 'tenoobendpointgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenoobendpointgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenoobendpointgroups" title="Oob Endpoint Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_oob_endpoint_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenoobendpointgroupsoobcontracts" title="Oob Contracts">
                        {isTenOobEndpointGroupsOobContractsActive && (  
                            <TenOobEndpointGroupsOobContracts
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].oob_contracts`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenoobendpointgroupsexpectedstate" title="Expected State">
                        {isTenOobEndpointGroupsExpectedStateActive && (  
                            <TenOobEndpointGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default TenOobEndpointGroups;

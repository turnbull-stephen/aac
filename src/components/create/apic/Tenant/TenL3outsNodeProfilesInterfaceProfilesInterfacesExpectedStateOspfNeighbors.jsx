...
const TenL3outsNodeProfilesInterfaceProfilesInterfacesExpectedStateOspfNeighbors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodeprofilesinterfaceprofilesinterfacesexpectedstateospfneighbors'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenl3outsnodeprofilesinterfaceprofilesinterfacesexpectedstateospfneighbors" title="Ospf Neighbors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_node_profiles_interface_profiles_interfaces_expected_state_ospf_neighbors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
...
export default TenL3outsNodeProfilesInterfaceProfilesInterfacesExpectedStateOspfNeighbors;

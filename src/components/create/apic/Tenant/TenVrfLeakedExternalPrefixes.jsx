...
import TenVrfLeakedExternalPrefixesDestinations from './TenVrfLeakedExternalPrefixesDestinations'


const TenVrfLeakedExternalPrefixes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Leaked External Prefixes'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenVrfLeakedExternalPrefixesDestinationsActive = activeKeys[item.id] === 'tenvrfleakedexternalprefixesdestinations';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenvrfleakedexternalprefixes'}
                    ...
                    >
                      <Tab eventKey="tenvrfleakedexternalprefixes" title="Leaked External Prefixes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_vrf_leaked_external_prefixes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenvrfleakedexternalprefixesdestinations" title="Destinations">
                        {isTenVrfLeakedExternalPrefixesDestinationsActive && (  
                            <TenVrfLeakedExternalPrefixesDestinations
                              key={item.id}
                              control={control}
...
export default TenVrfLeakedExternalPrefixes;

...
import TenL3outsNodeProfilesInterfaceProfilesOspf from './TenL3outsNodeProfilesInterfaceProfilesOspf'
import TenL3outsNodeProfilesInterfaceProfilesEigrp from './TenL3outsNodeProfilesInterfaceProfilesEigrp'
import TenL3outsNodeProfilesInterfaceProfilesInterfaces from './TenL3outsNodeProfilesInterfaceProfilesInterfaces'


const TenL3outsNodeProfilesInterfaceProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Interface Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodeProfilesInterfaceProfilesOspfActive = activeKeys[item.id] === 'tenl3outsnodeprofilesinterfaceprofilesospf';
          const isTenL3outsNodeProfilesInterfaceProfilesEigrpActive = activeKeys[item.id] === 'tenl3outsnodeprofilesinterfaceprofileseigrp';
          const isTenL3outsNodeProfilesInterfaceProfilesInterfacesActive = activeKeys[item.id] === 'tenl3outsnodeprofilesinterfaceprofilesinterfaces';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodeprofilesinterfaceprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenl3outsnodeprofilesinterfaceprofiles" title="Interface Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_node_profiles_interface_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsnodeprofilesinterfaceprofilesospf" title="Ospf">
                        {isTenL3outsNodeProfilesInterfaceProfilesOspfActive && (  
                            <TenL3outsNodeProfilesInterfaceProfilesOspf
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenl3outsnodeprofilesinterfaceprofileseigrp" title="Eigrp">
                        {isTenL3outsNodeProfilesInterfaceProfilesEigrpActive && (  
                            <TenL3outsNodeProfilesInterfaceProfilesEigrp
                              key={item.id}
                              control={control}
                        ...
                        {isTenL3outsNodeProfilesInterfaceProfilesInterfacesActive && (  
                            <TenL3outsNodeProfilesInterfaceProfilesInterfaces
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].interfaces`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenL3outsNodeProfilesInterfaceProfiles;

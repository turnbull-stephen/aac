...
import TenInbEndpointGroupsContracts from './TenInbEndpointGroupsContracts'
import TenInbEndpointGroupsExpectedState from './TenInbEndpointGroupsExpectedState'


const TenInbEndpointGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Inb Endpoint Groups'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenInbEndpointGroupsContractsActive = activeKeys[item.id] === 'teninbendpointgroupscontracts';
          const isTenInbEndpointGroupsExpectedStateActive = activeKeys[item.id] === 'teninbendpointgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'teninbendpointgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="teninbendpointgroups" title="Inb Endpoint Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_inb_endpoint_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="teninbendpointgroupscontracts" title="Contracts">
                        {isTenInbEndpointGroupsContractsActive && (  
                            <TenInbEndpointGroupsContracts
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="teninbendpointgroupsexpectedstate" title="Expected State">
                        {isTenInbEndpointGroupsExpectedStateActive && (  
                            <TenInbEndpointGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default TenInbEndpointGroups;

...
import NpUpdateGroup from './NpUpdateGroup';
import NpVpcGroup from './NpVpcGroup';
import NpNode from './NpNode';
import NpExpectedState from './NpExpectedState';
...
const NodePoliciesForm = () => {
      ...
      if (fileExtension === 'yaml' || fileExtension === 'yml') {
        ...
        reader.onload = (event) => {
          try {
            ...
            reset(loadedData.apic.node_policies);
            onSubmit(loadedData.apic.node_policies)
  ...
  // Define the Root for apic/node_policies
  ...
  const onSubmit = (data) => {
    ...
    root.apic.node_policiess = processedData;
    
    // To display the YAML representation of the node_policies array,
  ...
  const hasNodePoliciesErrors =Boolean(errors.inb_endpoint_group) || Boolean(errors.oob_endpoint_group);
  const hasNpUpdateGroupErrors = Boolean(errors.update_groups);
  const hasNpVpcGroupErrors = Boolean(errors.vpc_groups);
  const hasNpNodeErrors = Boolean(errors.nodes);
  const hasNpExpectedStateErrors = Boolean(errors.expected_state);
    ...
    <div className="full-height-layout">
      <Container fluid>
        <Row>
          <Col md={3}>
            <div>
              <label>Load Node Policies Data:</label>
          ...
          <Col md={9}>
            <div >
            <h3 className='mt-2'>Node Policies</h3>
                  ...
                  <Card>
                    <Card.Body>
                      ...
                      >
                        ...
                        <Tab eventKey="npupdategroup" title={tabTitle('Update Groups', hasNpUpdateGroupErrors)}>
                          <Card>
                            <Card.Body>
                            <NpUpdateGroup control={control} register={register} errors={errors} clearErrors={clearErrors} setError={setError} name={'update_groups'} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        <Tab eventKey="npvpcgroup" title={tabTitle('Vpc Groups', hasNpVpcGroupErrors)}>
                          <Card>
                            <Card.Body>
                            <NpVpcGroup control={control} register={register} errors={errors} clearErrors={clearErrors} setError={setError} name={'vpc_groups'} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        <Tab eventKey="npnode" title={tabTitle('Nodes', hasNpNodeErrors)}>
                          <Card>
                            <Card.Body>
                            <NpNode control={control} register={register} errors={errors} clearErrors={clearErrors} setError={setError} name={'nodes'} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        <Tab eventKey="npexpectedstate" title={tabTitle('Expected State', hasNpExpectedStateErrors)}>
                          <Card>
                            <Card.Body>
                            <NpExpectedState control={control} register={register} errors={errors} clearErrors={clearErrors} setError={setError} name={'expected_state'} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        </Tabs>
...
export default NodePoliciesForm;

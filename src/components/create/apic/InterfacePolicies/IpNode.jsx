...
import IpInterface from './IpInterface'
import IpFex from './IpFex'


const IpNode = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isIpInterfaceActive = activeKeys[item.id] === 'ipinterface';
          const isIpFexActive = activeKeys[item.id] === 'ipfex';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'ipnode'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="ipnode" title="Nodes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'interface_policies.ip_node', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="ipfex" title="Fexes">
                        {isIpFexActive && (  
                            <IpFex
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].fexes`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

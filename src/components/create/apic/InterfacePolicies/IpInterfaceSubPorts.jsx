...
import IpInterfaceSubPortsExpectedState from './IpInterfaceSubPortsExpectedState'


const IpInterfaceSubPorts = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isIpInterfaceSubPortsExpectedStateActive = activeKeys[item.id] === 'ipinterfacesubportsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'ipinterfacesubports'}
                    ...
                    >
                      <Tab eventKey="ipinterfacesubports" title="Sub Ports">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'interface_policies.ip_interface_sub_ports', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="ipinterfacesubportsexpectedstate" title="Expected State">
                        {isIpInterfaceSubPortsExpectedStateActive && (  
                            <IpInterfaceSubPortsExpectedState
                              key={item.id}
                              control={control}
...
export default IpInterfaceSubPorts;

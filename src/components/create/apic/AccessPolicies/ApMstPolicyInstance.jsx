...
import ApMstPolicyInstanceVlanRange from './ApMstPolicyInstanceVlanRange'


const ApMstPolicyInstance = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApMstPolicyInstanceVlanRangeActive = activeKeys[item.id] === 'apmstpolicyinstancevlanrange';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apmstpolicyinstance'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apmstpolicyinstance" title="Instances">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_mst_policy_instance', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apmstpolicyinstancevlanrange" title="Vlan Ranges">
                        {isApMstPolicyInstanceVlanRangeActive && (  
                            <ApMstPolicyInstanceVlanRange
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].vlan_ranges`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default ApMstPolicyInstance;

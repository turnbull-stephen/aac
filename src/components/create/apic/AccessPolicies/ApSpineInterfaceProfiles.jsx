...
import ApSpineInterfaceProfilesSelectors from './ApSpineInterfaceProfilesSelectors'
import ApSpineInterfaceProfilesExpectedState from './ApSpineInterfaceProfilesExpectedState'


const ApSpineInterfaceProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Spine Interface Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpineInterfaceProfilesSelectorsActive = activeKeys[item.id] === 'apspineinterfaceprofilesselectors';
          const isApSpineInterfaceProfilesExpectedStateActive = activeKeys[item.id] === 'apspineinterfaceprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspineinterfaceprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apspineinterfaceprofiles" title="Spine Interface Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_spine_interface_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspineinterfaceprofilesselectors" title="Selectors">
                        {isApSpineInterfaceProfilesSelectorsActive && (  
                            <ApSpineInterfaceProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apspineinterfaceprofilesexpectedstate" title="Expected State">
                        {isApSpineInterfaceProfilesExpectedStateActive && (  
                            <ApSpineInterfaceProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default ApSpineInterfaceProfiles;

...
import ApRoutedDomainExpectedState from './ApRoutedDomainExpectedState'


const ApRoutedDomain = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApRoutedDomainExpectedStateActive = activeKeys[item.id] === 'aprouteddomainexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'aprouteddomain'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="aprouteddomain" title="Routed Domains">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_routed_domain', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="aprouteddomainexpectedstate" title="Expected State">
                        {isApRoutedDomainExpectedStateActive && (  
                            <ApRoutedDomainExpectedState
                              key={item.id}
                              control={control}

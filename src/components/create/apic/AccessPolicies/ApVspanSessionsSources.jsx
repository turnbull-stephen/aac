...
import ApVspanSessionsSourcesAccessPaths from './ApVspanSessionsSourcesAccessPaths'


const ApVspanSessionsSources = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApVspanSessionsSourcesAccessPathsActive = activeKeys[item.id] === 'apvspansessionssourcesaccesspaths';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apvspansessionssources'}
                    ...
                    >
                      <Tab eventKey="apvspansessionssources" title="Sources">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_vspan_sessions_sources', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apvspansessionssourcesaccesspaths" title="Access Paths">
                        {isApVspanSessionsSourcesAccessPathsActive && (  
                            <ApVspanSessionsSourcesAccessPaths
                              key={item.id}
                              control={control}
...
export default ApVspanSessionsSources;

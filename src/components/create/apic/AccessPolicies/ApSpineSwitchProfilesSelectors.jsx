...
import ApSpineSwitchProfilesSelectorsNodeBlocks from './ApSpineSwitchProfilesSelectorsNodeBlocks'


const ApSpineSwitchProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpineSwitchProfilesSelectorsNodeBlocksActive = activeKeys[item.id] === 'apspineswitchprofilesselectorsnodeblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspineswitchprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="apspineswitchprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_spine_switch_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspineswitchprofilesselectorsnodeblocks" title="Node Blocks">
                        {isApSpineSwitchProfilesSelectorsNodeBlocksActive && (  
                            <ApSpineSwitchProfilesSelectorsNodeBlocks
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].node_blocks`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default ApSpineSwitchProfilesSelectors;

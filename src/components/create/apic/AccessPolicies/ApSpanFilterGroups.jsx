...
import ApSpanFilterGroupsEntries from './ApSpanFilterGroupsEntries'


const ApSpanFilterGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpanFilterGroupsEntriesActive = activeKeys[item.id] === 'apspanfiltergroupsentries';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspanfiltergroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apspanfiltergroups" title="Filter Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_span_filter_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspanfiltergroupsentries" title="Entries">
                        {isApSpanFilterGroupsEntriesActive && (  
                            <ApSpanFilterGroupsEntries
                              key={item.id}
                              control={control}
...
export default ApSpanFilterGroups;

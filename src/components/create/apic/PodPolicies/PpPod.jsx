...
import PpPodExternalTepPools from './PpPodExternalTepPools'
import PpPodRemotePools from './PpPodRemotePools'
import PpPodExpectedState from './PpPodExpectedState'


const PpPod = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isPpPodExternalTepPoolsActive = activeKeys[item.id] === 'pppodexternalteppools';
          const isPpPodRemotePoolsActive = activeKeys[item.id] === 'pppodremotepools';
          const isPpPodExpectedStateActive = activeKeys[item.id] === 'pppodexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'pppod'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="pppod" title="Pods">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'pod_policies.pp_pod', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="pppodexternalteppools" title="External Tep Pools">
                        {isPpPodExternalTepPoolsActive && (  
                            <PpPodExternalTepPools
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].external_tep_pools`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="pppodremotepools" title="Remote Pools">
                        {isPpPodRemotePoolsActive && (  
                            <PpPodRemotePools
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].remote_pools`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="pppodexpectedstate" title="Expected State">
                        {isPpPodExpectedStateActive && (  
                            <PpPodExpectedState
                              key={item.id}
                              control={control}

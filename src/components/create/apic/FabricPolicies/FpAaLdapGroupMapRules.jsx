...
import FpAaLdapGroupMapRulesSecurityDomains from './FpAaLdapGroupMapRulesSecurityDomains'


const FpAaLdapGroupMapRules = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaLdapGroupMapRulesSecurityDomainsActive = activeKeys[item.id] === 'fpaaldapgroupmaprulessecuritydomains';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaldapgroupmaprules'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaldapgroupmaprules" title="Group Map Rules">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aa_ldap_group_map_rules', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isFpAaLdapGroupMapRulesSecurityDomainsActive && (  
                            <FpAaLdapGroupMapRulesSecurityDomains
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].security_domains`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpAaLdapGroupMapRules;

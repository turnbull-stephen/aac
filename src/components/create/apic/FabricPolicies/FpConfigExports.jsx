...
import FpConfigExportsExpectedState from './FpConfigExportsExpectedState'


const FpConfigExports = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpConfigExportsExpectedStateActive = activeKeys[item.id] === 'fpconfigexportsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpconfigexports'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpconfigexports" title="Config Exports">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_config_exports', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpconfigexportsexpectedstate" title="Expected State">
                        {isFpConfigExportsExpectedStateActive && (  
                            <FpConfigExportsExpectedState
                              key={item.id}
                              control={control}
...
export default FpConfigExports;

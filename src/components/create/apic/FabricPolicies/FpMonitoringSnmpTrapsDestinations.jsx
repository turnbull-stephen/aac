...
const FpMonitoringSnmpTrapsDestinations = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpmonitoringsnmptrapsdestinations'}
                        ...
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_monitoring_snmp_traps_destinations', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
...
export default FpMonitoringSnmpTrapsDestinations;

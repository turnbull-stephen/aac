...
import FpAaaTacacsProvidersExpectedState from './FpAaaTacacsProvidersExpectedState'


const FpAaaTacacsProviders = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaaTacacsProvidersExpectedStateActive = activeKeys[item.id] === 'fpaaatacacsprovidersexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaatacacsproviders'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaatacacsproviders" title="Tacacs Providers">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aaa_tacacs_providers', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaatacacsprovidersexpectedstate" title="Expected State">
                        {isFpAaaTacacsProvidersExpectedStateActive && (  
                            <FpAaaTacacsProvidersExpectedState
                              key={item.id}
                              control={control}
...
export default FpAaaTacacsProviders;

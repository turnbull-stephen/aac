...
import FpMonitoringSmartCallhomesDestinations from './FpMonitoringSmartCallhomesDestinations'


const FpMonitoringSmartCallhomes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpMonitoringSmartCallhomesDestinationsActive = activeKeys[item.id] === 'fpmonitoringsmartcallhomesdestinations';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpmonitoringsmartcallhomes'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpmonitoringsmartcallhomes" title="Smart Callhomes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_monitoring_smart_callhomes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpmonitoringsmartcallhomesdestinations" title="Destinations">
                        {isFpMonitoringSmartCallhomesDestinationsActive && (  
                            <FpMonitoringSmartCallhomesDestinations
                              key={item.id}
                              control={control}
...
export default FpMonitoringSmartCallhomes;

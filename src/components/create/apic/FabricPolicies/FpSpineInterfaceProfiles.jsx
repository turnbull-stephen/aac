...
import FpSpineInterfaceProfilesSelectors from './FpSpineInterfaceProfilesSelectors'
import FpSpineInterfaceProfilesExpectedState from './FpSpineInterfaceProfilesExpectedState'


const FpSpineInterfaceProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpineInterfaceProfilesSelectorsActive = activeKeys[item.id] === 'fpspineinterfaceprofilesselectors';
          const isFpSpineInterfaceProfilesExpectedStateActive = activeKeys[item.id] === 'fpspineinterfaceprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspineinterfaceprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpspineinterfaceprofiles" title="Spine Interface Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_spine_interface_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspineinterfaceprofilesselectors" title="Selectors">
                        {isFpSpineInterfaceProfilesSelectorsActive && (  
                            <FpSpineInterfaceProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="fpspineinterfaceprofilesexpectedstate" title="Expected State">
                        {isFpSpineInterfaceProfilesExpectedStateActive && (  
                            <FpSpineInterfaceProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default FpSpineInterfaceProfiles;

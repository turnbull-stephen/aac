...
const  FpSmartLicensingProxy = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpsmartlicensingproxy'); // Keep state for the Tab selection
  ...
  const isFpSmartLicensingProxyActive = key === 'fpsmartlicensingproxy';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fpsmartlicensingproxy" title={tabTitle('Proxy', hasFpSmartLicensingProxyErrors)}>
          {isFpSmartLicensingProxyActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_smart_licensing_proxy', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpSmartLicensingProxy;

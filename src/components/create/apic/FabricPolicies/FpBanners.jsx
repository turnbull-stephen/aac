...
const  FpBanners = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpbanners'); // Keep state for the Tab selection
  ...
  const hasFpBannersErrors = Boolean(errors && errors[name] && errors[name].banners)//;
  
  const isFpBannersActive = key === 'fpbanners';
  return (
    <div>
            ...
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_banners', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

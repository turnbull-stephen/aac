...
const  FpEpLoopProtection = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpeploopprotection'); // Keep state for the Tab selection
  ...
  const hasFpEpLoopProtectionErrors = Boolean(errors && errors[name] && errors[name].ep_loop_protection)//;
  
  const isFpEpLoopProtectionActive = key === 'fpeploopprotection';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fpeploopprotection" title={tabTitle('Ep Loop Protection', hasFpEpLoopProtectionErrors)}>
          {isFpEpLoopProtectionActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_ep_loop_protection', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpEpLoopProtection;

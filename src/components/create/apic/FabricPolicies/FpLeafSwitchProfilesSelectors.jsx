...
import FpLeafSwitchProfilesSelectorsNodeBlocks from './FpLeafSwitchProfilesSelectorsNodeBlocks'


const FpLeafSwitchProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpLeafSwitchProfilesSelectorsNodeBlocksActive = activeKeys[item.id] === 'fpleafswitchprofilesselectorsnodeblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpleafswitchprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="fpleafswitchprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_leaf_switch_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpleafswitchprofilesselectorsnodeblocks" title="Node Blocks">
                        {isFpLeafSwitchProfilesSelectorsNodeBlocksActive && (  
                            <FpLeafSwitchProfilesSelectorsNodeBlocks
                              key={item.id}
                              control={control}
...
export default FpLeafSwitchProfilesSelectors;

...
import FpGeolocationSitesBuildingsFloorsRoomsRowsRacks from './FpGeolocationSitesBuildingsFloorsRoomsRowsRacks'


const FpGeolocationSitesBuildingsFloorsRoomsRows = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpGeolocationSitesBuildingsFloorsRoomsRowsRacksActive = activeKeys[item.id] === 'fpgeolocationsitesbuildingsfloorsroomsrowsracks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpgeolocationsitesbuildingsfloorsroomsrows'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpgeolocationsitesbuildingsfloorsroomsrows" title="Rows">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_geolocation_sites_buildings_floors_rooms_rows', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpgeolocationsitesbuildingsfloorsroomsrowsracks" title="Racks">
                        {isFpGeolocationSitesBuildingsFloorsRoomsRowsRacksActive && (  
                            <FpGeolocationSitesBuildingsFloorsRoomsRowsRacks
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].racks`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpGeolocationSitesBuildingsFloorsRoomsRows;

...
import FpPodProfilesSelectors from './FpPodProfilesSelectors'
import FpPodProfilesExpectedState from './FpPodProfilesExpectedState'


const FpPodProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpPodProfilesSelectorsActive = activeKeys[item.id] === 'fppodprofilesselectors';
          const isFpPodProfilesExpectedStateActive = activeKeys[item.id] === 'fppodprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fppodprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fppodprofiles" title="Pod Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_pod_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fppodprofilesselectors" title="Selectors">
                        {isFpPodProfilesSelectorsActive && (  
                            <FpPodProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="fppodprofilesexpectedstate" title="Expected State">
                        {isFpPodProfilesExpectedStateActive && (  
                            <FpPodProfilesExpectedState
                              key={item.id}
                              control={control}

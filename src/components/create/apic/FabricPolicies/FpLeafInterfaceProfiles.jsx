...
import FpLeafInterfaceProfilesExpectedState from './FpLeafInterfaceProfilesExpectedState'


const FpLeafInterfaceProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpLeafInterfaceProfilesExpectedStateActive = activeKeys[item.id] === 'fpleafinterfaceprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpleafinterfaceprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpleafinterfaceprofiles" title="Leaf Interface Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_leaf_interface_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpleafinterfaceprofilesexpectedstate" title="Expected State">
                        {isFpLeafInterfaceProfilesExpectedStateActive && (  
                            <FpLeafInterfaceProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default FpLeafInterfaceProfiles;

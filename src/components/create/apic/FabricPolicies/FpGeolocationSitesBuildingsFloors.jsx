...
import FpGeolocationSitesBuildingsFloorsRooms from './FpGeolocationSitesBuildingsFloorsRooms'


const FpGeolocationSitesBuildingsFloors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpGeolocationSitesBuildingsFloorsRoomsActive = activeKeys[item.id] === 'fpgeolocationsitesbuildingsfloorsrooms';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpgeolocationsitesbuildingsfloors'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpgeolocationsitesbuildingsfloors" title="Floors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_geolocation_sites_buildings_floors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isFpGeolocationSitesBuildingsFloorsRoomsActive && (  
                            <FpGeolocationSitesBuildingsFloorsRooms
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].rooms`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpGeolocationSitesBuildingsFloors;

...
import FpRemoteLocationsExpectedState from './FpRemoteLocationsExpectedState'


const FpRemoteLocations = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpRemoteLocationsExpectedStateActive = activeKeys[item.id] === 'fpremotelocationsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpremotelocations'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpremotelocations" title="Remote Locations">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_remote_locations', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpremotelocationsexpectedstate" title="Expected State">
                        {isFpRemoteLocationsExpectedStateActive && (  
                            <FpRemoteLocationsExpectedState
                              key={item.id}
                              control={control}
...
export default FpRemoteLocations;

...
const  FpGlobalSettings = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpglobalsettings'); // Keep state for the Tab selection
  ...
  const hasFpGlobalSettingsErrors = Boolean(errors && errors[name] && errors[name].global_settings)//;
  
  const isFpGlobalSettingsActive = key === 'fpglobalsettings';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fpglobalsettings" title={tabTitle('Global Settings', hasFpGlobalSettingsErrors)}>
          {isFpGlobalSettingsActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_global_settings', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpGlobalSettings;

...
import FpAaLdapGroupMapRulesSecurityDomainsRoles from './FpAaLdapGroupMapRulesSecurityDomainsRoles'


const FpAaLdapGroupMapRulesSecurityDomains = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaLdapGroupMapRulesSecurityDomainsRolesActive = activeKeys[item.id] === 'fpaaldapgroupmaprulessecuritydomainsroles';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaldapgroupmaprulessecuritydomains'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaldapgroupmaprulessecuritydomains" title="Security Domains">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aa_ldap_group_map_rules_security_domains', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaldapgroupmaprulessecuritydomainsroles" title="Roles">
                        {isFpAaLdapGroupMapRulesSecurityDomainsRolesActive && (  
                            <FpAaLdapGroupMapRulesSecurityDomainsRoles
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].roles`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpAaLdapGroupMapRulesSecurityDomains;

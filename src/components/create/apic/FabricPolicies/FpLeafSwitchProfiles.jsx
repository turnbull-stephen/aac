...
import FpLeafSwitchProfilesSelectors from './FpLeafSwitchProfilesSelectors'
import FpLeafSwitchProfilesExpectedState from './FpLeafSwitchProfilesExpectedState'


const FpLeafSwitchProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Leaf Switch Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpLeafSwitchProfilesSelectorsActive = activeKeys[item.id] === 'fpleafswitchprofilesselectors';
          const isFpLeafSwitchProfilesExpectedStateActive = activeKeys[item.id] === 'fpleafswitchprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpleafswitchprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpleafswitchprofiles" title="Leaf Switch Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_leaf_switch_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpleafswitchprofilesselectors" title="Selectors">
                        {isFpLeafSwitchProfilesSelectorsActive && (  
                            <FpLeafSwitchProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="fpleafswitchprofilesexpectedstate" title="Expected State">
                        {isFpLeafSwitchProfilesExpectedStateActive && (  
                            <FpLeafSwitchProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default FpLeafSwitchProfiles;

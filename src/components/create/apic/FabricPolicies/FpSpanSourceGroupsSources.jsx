...
import FpSpanSourceGroupsSourcesAccessPaths from './FpSpanSourceGroupsSourcesAccessPaths'


const FpSpanSourceGroupsSources = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpanSourceGroupsSourcesAccessPathsActive = activeKeys[item.id] === 'fpspansourcegroupssourcesaccesspaths';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspansourcegroupssources'}
                    ...
                    >
                      <Tab eventKey="fpspansourcegroupssources" title="Sources">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_span_source_groups_sources', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspansourcegroupssourcesaccesspaths" title="Fabric Paths">
                        {isFpSpanSourceGroupsSourcesAccessPathsActive && (  
                            <FpSpanSourceGroupsSourcesAccessPaths
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].fabric_paths`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpSpanSourceGroupsSources;

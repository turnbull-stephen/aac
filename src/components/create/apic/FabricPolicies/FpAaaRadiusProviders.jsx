...
import FpAaaRadiusProvidersExpectedState from './FpAaaRadiusProvidersExpectedState'


const FpAaaRadiusProviders = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaaRadiusProvidersExpectedStateActive = activeKeys[item.id] === 'fpaaaradiusprovidersexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaaradiusproviders'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaaradiusproviders" title="Radius Providers">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aaa_radius_providers', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaaradiusprovidersexpectedstate" title="Expected State">
                        {isFpAaaRadiusProvidersExpectedStateActive && (  
                            <FpAaaRadiusProvidersExpectedState
                              key={item.id}
                              control={control}
...
export default FpAaaRadiusProviders;

...
import FpSpanSourceGroupsSources from './FpSpanSourceGroupsSources'
import FpSpanSourceGroupsDestination from './FpSpanSourceGroupsDestination'
import FpSpanSourceGroupsExpectedState from './FpSpanSourceGroupsExpectedState'


const FpSpanSourceGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpanSourceGroupsSourcesActive = activeKeys[item.id] === 'fpspansourcegroupssources';
          const isFpSpanSourceGroupsDestinationActive = activeKeys[item.id] === 'fpspansourcegroupsdestination';
          const isFpSpanSourceGroupsExpectedStateActive = activeKeys[item.id] === 'fpspansourcegroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspansourcegroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpspansourcegroups" title="Source Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_span_source_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspansourcegroupssources" title="Sources">
                        {isFpSpanSourceGroupsSourcesActive && (  
                            <FpSpanSourceGroupsSources
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="fpspansourcegroupsdestination" title="Destination">
                        {isFpSpanSourceGroupsDestinationActive && (  
                            <FpSpanSourceGroupsDestination
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="fpspansourcegroupsexpectedstate" title="Expected State">
                        {isFpSpanSourceGroupsExpectedStateActive && (  
                            <FpSpanSourceGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default FpSpanSourceGroups;

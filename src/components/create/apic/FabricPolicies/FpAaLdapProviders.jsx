...
import FpAaLdapProvidersExpectedState from './FpAaLdapProvidersExpectedState'


const FpAaLdapProviders = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaLdapProvidersExpectedStateActive = activeKeys[item.id] === 'fpaaldapprovidersexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaldapproviders'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaldapproviders" title="Providers">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aa_ldap_providers', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaldapprovidersexpectedstate" title="Expected State">
                        {isFpAaLdapProvidersExpectedStateActive && (  
                            <FpAaLdapProvidersExpectedState
                              key={item.id}
                              control={control}
...
export default FpAaLdapProviders;

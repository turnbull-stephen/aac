...
import FpGeolocationSitesBuildingsFloors from './FpGeolocationSitesBuildingsFloors'


const FpGeolocationSitesBuildings = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpGeolocationSitesBuildingsFloorsActive = activeKeys[item.id] === 'fpgeolocationsitesbuildingsfloors';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpgeolocationsitesbuildings'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpgeolocationsitesbuildings" title="Buildings">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_geolocation_sites_buildings', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isFpGeolocationSitesBuildingsFloorsActive && (  
                            <FpGeolocationSitesBuildingsFloors
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].floors`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpGeolocationSitesBuildings;

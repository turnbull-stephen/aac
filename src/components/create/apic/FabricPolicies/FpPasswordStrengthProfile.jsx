...
const  FpPasswordStrengthProfile = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fppasswordstrengthprofile'); // Keep state for the Tab selection
  ...
  const isFpPasswordStrengthProfileActive = key === 'fppasswordstrengthprofile';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fppasswordstrengthprofile" title={tabTitle('Password Strength Profile', hasFpPasswordStrengthProfileErrors)}>
          {isFpPasswordStrengthProfileActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_password_strength_profile', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpPasswordStrengthProfile;

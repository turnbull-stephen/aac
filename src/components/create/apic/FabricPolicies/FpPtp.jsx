...
const  FpPtp = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpptp'); // Keep state for the Tab selection
  ...
  const hasFpPtpErrors = Boolean(errors && errors[name] && errors[name].ptp)//;
  
  const isFpPtpActive = key === 'fpptp';
  return (
    <div>
            ...
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_ptp', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

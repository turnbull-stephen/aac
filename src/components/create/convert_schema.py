...
with open('./apic/apic_schema.yaml', 'r') as file:
    # Create a generator for the documents
    aac_schema = yaml.safe_load(file)
...
apic_parents = ["fabric_policies", "access_policies", "pod_policies", "node_policies", "interface_policies", "tenant", "apic"]

def add_to_parents(schema_name):
    object_name = schema_name.split('_')[0]
    # For APIC Schema
    if object_name == 'ten' or schema_name == 'tenant':
        return 'tenant'
    if object_name == 'ap' or schema_name == 'access_policies':
        return 'access_policies'
    if object_name == 'fp' or schema_name == 'fabric_policies':
        return 'fabric_policies'
    if object_name == 'pp' or schema_name == 'pod_policies':
        return 'pod_policies'
    if object_name == 'np' or schema_name == 'node_policies':
        return 'node_policies'
    if object_name == 'ip' or schema_name == 'interface_policies':
        return 'interface_policies'
    if object_name == 'bootstrap' or schema_name == 'expected_state' or schema_name == 'apic':
        return 'apic'

new_schema = {}  # Ensure new_schema is initialized

for parent in apic_parents:
    new_schema.update({parent : {}})

for schema_name, attributes in aac_schema.items():
        ...
        for attribute_name, validation_rule in attributes.items():
            ...
            if validation_rule.split('(')[0] ==  "regex":
                ...
                new_schema[parent][schema_name][attribute_name]['pattern'] = pattern.replace('^[a-zA-Z0-9_.:-]{1,64}$', '^[a-zA-Z0-9_.:-]+$').replace("^[a-zA-Z0-9\\\\!#$%()*,-./:;@ _{|}~?&+]{1,128}$", '^[a-zA-Z0-9!#$%()*,-./:;@ _{|}~?&+]+$').replace('^[a-zA-Z0-9\\\\\\\\!#$%()*,-./:;@ _{|}~?&+]{1,128}$', '^[a-zA-Z0-9!#$%()*,-./:;@ _{|}~?&+]+$').replace("[a-zA-Z0-9\\\\\\\\!#$%()*,-./:;@ _{|}~?&+]{1,128}$", '^[a-zA-Z0-9!#$%()*,-./:;@ _{|}~?&+]+$').replace('^[a-zA-Z0-9\\\\\\\\!#$%()*,-./:;@ _{|}~?&+]{1,128}$', '^[a-zA-Z0-9!#$%()*,-./:;@ _{|}~?&+]+$').replace("^[a-zA-Z0-9\\\\\\\\!#$%()*,-./:;@ _{|}~?&+]{1,64}$", '^[a-zA-Z0-9_.:-]+$')
...
normalize_title = ['Apic', 'Ep', 'Ptp', 'Ip', 'Coop', 'Isis', 'Bfd', 'As', 'Bgp', 'Dns', 'Dscp', 'Vmm', 'Aaa', 'Mtu', 'Gui', 'Cli', 'Vrf', 'Ospf', 'Eigrp', 'Igmp', 'Id', 'Svi', 'Vlan', 'Ebgp', 'Ibgp', 'Elag', 'Ttl', 'Mpls', 'Sr', 'Qos', 'Nd', 'Lsa', 'Spf', 'Asm', 'Sla', 'Arp', 'Nd', 'Ra', 'Vm',  ]
        ...
        for object_name, object_attributes in attributes.items():
            ...
            if isinstance(object_attributes, dict):
                ...
                for attribute_name, attribute_item in object_attributes.items():
                    ...
                    # Check for the 'title' key and update if necessary
                    if attribute_item.get('title'):
                        title = attribute_item.get('title')
                        print(title)
                        # Split the title into words and capitalize if needed
                        title_words = title.split()
                        title_words = [word.upper() if word.capitalize() in normalize_title else word for word in title_words]
                        # Join the words back into a string and update the title
                        updated_title = ' '.join(title_words)
                        attribute_item['title'] = updated_title

                    # Update the customized dictionary with the modified attribute_item
                    updated_attributes[attribute_name] = attribute_item
...
#with open(r'./apic/aac_apic_custom_template_schema.json', 'w') as file:
#    documents = json.dump(customized, file, indent=2)
    


#int_list = list(dict.fromkeys(int_list))

#print(int_list)

with open('./apic/aac_apic_schema_additions.yaml', 'r') as file:
    # Create a generator for the documents
...
with open(r'./apic/aac_apic_schema.yaml', 'w') as file:
    documents = yaml.dump(new_schema, file)
#    
with open(r'./apic/aac_apic_schema.json', 'w') as file:
    documents = json.dump(new_schema, file, indent=2)
#
#
#for schema_name, attributes in new_schema.items():
#    with open('./apic/'+schema_name + '.json', 'w') as file:

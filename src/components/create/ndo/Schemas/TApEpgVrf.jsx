...
const  TApEpgVrf = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('tapepgvrf'); // Keep state for the Tab selection
  ...
  const hasTApEpgVrfErrors = Boolean(errors && errors[name] && errors[name].vrf)//;
  
  const isTApEpgVrfActive = key === 'tapepgvrf';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        
        
        <Tab eventKey="tapepgvrf" title={tabTitle('Vrf', hasTApEpgVrfErrors)}>
          {isTApEpgVrfActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'schemas.t_ap_epg_vrf', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

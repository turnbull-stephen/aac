...
import TVrfSitesRegionsCidrs from './TVrfSitesRegionsCidrs'


const TVrfSitesRegions = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTVrfSitesRegionsCidrsActive = activeKeys[item.id] === 'tvrfsitesregionscidrs';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tvrfsitesregions'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tvrfsitesregions" title="Regions">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_vrf_sites_regions', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isTVrfSitesRegionsCidrsActive && (  
                            <TVrfSitesRegionsCidrs
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].cidrs`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TVrfSitesRegions;

...
import TApEpgBridgeDomain from './TApEpgBridgeDomain'
import TApEpgVrf from './TApEpgVrf'
import TApEpgSubnets from './TApEpgSubnets'
import TApEpgContracts from './TApEpgContracts'
import TApEpgSites from './TApEpgSites'


const TApEndpointGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTApEpgBridgeDomainActive = activeKeys[item.id] === 'tapepgbridgedomain';
          const isTApEpgVrfActive = activeKeys[item.id] === 'tapepgvrf';
          const isTApEpgSubnetsActive = activeKeys[item.id] === 'tapepgsubnets';
          const isTApEpgContractsActive = activeKeys[item.id] === 'tapepgcontracts';
          const isTApEpgSitesActive = activeKeys[item.id] === 'tapepgsites';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tapendpointgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tapendpointgroups" title="Endpoint Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_ap_endpoint_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tapepgbridgedomain" title="Bridge Domain">
                        {isTApEpgBridgeDomainActive && (  
                            <TApEpgBridgeDomain
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].bridge_domain`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tapepgvrf" title="Vrf">
                        {isTApEpgVrfActive && (  
                            <TApEpgVrf
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tapepgsubnets" title="Subnets">
                        {isTApEpgSubnetsActive && (  
                            <TApEpgSubnets
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tapepgcontracts" title="Contracts">
                        {isTApEpgContractsActive && (  
                            <TApEpgContracts
                              key={item.id}
                              control={control}
...
export default TApEndpointGroups;

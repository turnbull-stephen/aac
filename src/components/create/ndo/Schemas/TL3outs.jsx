...
import TL3outsVrf from './TL3outsVrf'


const TL3outs = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTL3outsVrfActive = activeKeys[item.id] === 'tl3outsvrf';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tl3outs'}
                    ...
                    >
                      <Tab eventKey="tl3outs" title="L3outs">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_l3outs', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tl3outsvrf" title="Vrf">
                        {isTL3outsVrfActive && (  
                            <TL3outsVrf
                              key={item.id}
                              control={control}

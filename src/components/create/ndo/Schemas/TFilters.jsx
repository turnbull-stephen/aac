...
import TFiltersEntries from './TFiltersEntries'


const TFilters = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTFiltersEntriesActive = activeKeys[item.id] === 'tfiltersentries';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tfilters'}
                    ...
                    >
                      <Tab eventKey="tfilters" title="Filters">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_filters', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tfiltersentries" title="Entries">
                        {isTFiltersEntriesActive && (  
                            <TFiltersEntries
                              key={item.id}
                              control={control}

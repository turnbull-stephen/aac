...
import SitesPodsSpinesInterfacesOspf from './SitesPodsSpinesInterfacesOspf'


const SitesPodsSpinesInterfaces = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isSitesPodsSpinesInterfacesOspfActive = activeKeys[item.id] === 'sitespodsspinesinterfacesospf';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'sitespodsspinesinterfaces'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="sitespodsspinesinterfaces" title="Interfaces">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.sites_pods_spines_interfaces', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="sitespodsspinesinterfacesospf" title="Ospf">
                        {isSitesPodsSpinesInterfacesOspfActive && (  
                            <SitesPodsSpinesInterfacesOspf
                              key={item.id}
                              control={control}
...
export default SitesPodsSpinesInterfaces;

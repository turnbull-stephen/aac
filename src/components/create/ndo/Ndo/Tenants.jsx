...
import TenantsSites from './TenantsSites'
import TenantsUsers from './TenantsUsers'


const Tenants = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenantsSitesActive = activeKeys[item.id] === 'tenantssites';
          const isTenantsUsersActive = activeKeys[item.id] === 'tenantsusers';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenants'}
                    ...
                    >
                      <Tab eventKey="tenants" title="Tenants">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.tenants', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenantssites" title="Sites">
                        {isTenantsSitesActive && (  
                            <TenantsSites
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenantsusers" title="Users">
                        {isTenantsUsersActive && (  
                            <TenantsUsers
                              key={item.id}
                              control={control}

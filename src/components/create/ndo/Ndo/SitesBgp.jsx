...
const  SitesBgp = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('sitesbgp'); // Keep state for the Tab selection
  ...
  const hasSitesBgpErrors = Boolean(errors && errors[name] && errors[name].bgp)//;
  
  const isSitesBgpActive = key === 'sitesbgp';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        
        
        <Tab eventKey="sitesbgp" title={tabTitle('Bgp', hasSitesBgpErrors)}>
          {isSitesBgpActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'ndo.sites_bgp', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

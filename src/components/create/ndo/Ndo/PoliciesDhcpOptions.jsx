...
import PoliciesDhcpOptionsOptions from './PoliciesDhcpOptionsOptions'


const PoliciesDhcpOptions = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isPoliciesDhcpOptionsOptionsActive = activeKeys[item.id] === 'policiesdhcpoptionsoptions';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'policiesdhcpoptions'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="policiesdhcpoptions" title="Dhcp Options">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.policies_dhcp_options', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="policiesdhcpoptionsoptions" title="Options">
                        {isPoliciesDhcpOptionsOptionsActive && (  
                            <PoliciesDhcpOptionsOptions
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].options`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default PoliciesDhcpOptions;

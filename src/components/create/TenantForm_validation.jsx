...
const TenantForm = () => {
  ...
  const handleValidationCheckboxChange = (event) => {
  ...
  const [activeKeys, setActiveKeys] = useState('tenant');
  //const handleTabChange = (itemId, key) => {
  //  setActiveKeys(prevKeys => ({ ...prevKeys, [itemId]: key }));
  //};

  const isTenVrfActive = activeKeys === 'tenvrf';
  const isTenBridgeDomainsActive = activeKeys === 'tenbridgedomains';
  const isTenApplicationProfilesActive = activeKeys === 'tenapplicationprofiles';
  const isTenInbEndpointGroupsActive = activeKeys === 'teninb_endpoint_groups';
  const isTenOobEndpointGroupsActive = activeKeys === 'tenoobendpointgroups';
  const isTenExtMgmtInstancesActive = activeKeys === 'tenextmgmtinstances';
  const isTenL3outsActive = activeKeys === 'tenl3outs';
  const isTenSrMplsL3outsActive = activeKeys === 'tensrmplsl3outs';
  const isTenFiltersActive = activeKeys === 'tenfilters';
  const isTenContractsActive = activeKeys === 'tencontracts';
  const isTenOobContractsActive = activeKeys === 'tenoobcontracts';
  const isTenImportedContractsActive = activeKeys === 'tenimportedcontracts';
  const isTenPoliciesActive = activeKeys === 'tenpolicies';
  const isTenServicesActive = activeKeys === 'tenservices';
                    ...
                    <Card.Body>
                      ...
                      >
                        ...
                        <Tab eventKey="tenapplicationprofiles" title={tabTitle('Application Profiles', hasTenApplicationProfilesErrors)}>
                          {isTenApplicationProfilesActive && (
                            <Card>
                              <Card.Body>

...
const ObjectFunctions = ({
  ...
  if (value.type === 'string') {
    ...
    // check to see if  name is null/undefined - for errors handling
    const errorPath = name ? `${name}.${objectKey}` : objectKey;
    return (
      <div key={inputId} className="mb-0">
        ...
        />
        {_.get(errors, errorPath) && (
          <p role="alert" className='errors'>{_.get(errors, `${errorPath}.message`)}</p>
        )}
      </div>
    )
  }
  ...
  if (value.type === 'ip' || value.type === 'cidr' || value.type === 'ip_cidr') {
    ...
    return (
      <div key={`${name}.${objectKey}`} className="mb-0">
        <label htmlFor={inputId} className="form-label mb-0">
          {value.title}
          {isRequired && <span className="required">*</span>}
        </label>
  ...
  if (value.type === 'list-string') {
    const isRequired = value.required;
    // if name is null/undefined change to expected value. In this case tenant
    // check to see if  name is null/undefined - for errors handling
    const errorPath = name ? `${name}.${objectKey}` : objectKey;
    const inputId = name ? `${name}-${objectKey}` : objectKey
    const options = []
                ...
                if (option) {
                  if (value.oneOf[1].enum.includes(option.value)) {
                    ...
                    if (!isNaN(numValue) && (numValue >= value.oneOf[0].minimum && numValue <= value.oneOf[0].maximum)) {
                      ...
                      setError(`${name}.${objectKey}`, { type: "manual", message: 'Invalid entry. Please enter a number between: ' + value.oneOf[0].minimum + ' and ' + value.oneOf[0].maximum + '.' });
                    }
                  }
                } else {
                  clearErrors(`${name}.${objectKey}`);
                  field.onChange('');
                  if (isRequired) {
                    setError(`${name}.${objectKey}`, { type: "manual", message: `${value.title} is required` });
                  }
                }
              }}

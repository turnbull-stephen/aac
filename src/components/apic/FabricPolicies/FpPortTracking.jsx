...
const  FpPortTracking = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpporttracking'); // Keep state for the Tab selection
  ...
  const hasFpPortTrackingErrors = Boolean(errors && errors[name] && errors[name].port_tracking)//;
  
  const isFpPortTrackingActive = key === 'fpporttracking';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fpporttracking" title={tabTitle('Port Tracking', hasFpPortTrackingErrors)}>
          {isFpPortTrackingActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_port_tracking', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

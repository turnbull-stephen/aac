...
import FpAaaUsersDomainsRoles from './FpAaaUsersDomainsRoles'


const FpAaaUsersDomains = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaaUsersDomainsRolesActive = activeKeys[item.id] === 'fpaaausersdomainsroles';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaausersdomains'}
                    ...
                    >
                      <Tab eventKey="fpaaausersdomains" title="Domains">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aaa_users_domains', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaausersdomainsroles" title="Roles">
                        {isFpAaaUsersDomainsRolesActive && (  
                            <FpAaaUsersDomainsRoles
                              key={item.id}
                              control={control}
...
export default FpAaaUsersDomains;

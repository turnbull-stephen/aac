...
import FpSpineSwitchProfilesSelectors from './FpSpineSwitchProfilesSelectors'
import FpSpineSwitchProfilesExpectedState from './FpSpineSwitchProfilesExpectedState'


const FpSpineSwitchProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Spine Switch Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpineSwitchProfilesSelectorsActive = activeKeys[item.id] === 'fpspineswitchprofilesselectors';
          const isFpSpineSwitchProfilesExpectedStateActive = activeKeys[item.id] === 'fpspineswitchprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspineswitchprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpspineswitchprofiles" title="Spine Switch Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_spine_switch_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspineswitchprofilesselectors" title="Selectors">
                        {isFpSpineSwitchProfilesSelectorsActive && (  
                            <FpSpineSwitchProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="fpspineswitchprofilesexpectedstate" title="Expected State">
                        {isFpSpineSwitchProfilesExpectedStateActive && (  
                            <FpSpineSwitchProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default FpSpineSwitchProfiles;

...
import FpSpineInterfaceProfilesSelectorsPortBlocks from './FpSpineInterfaceProfilesSelectorsPortBlocks'


const FpSpineInterfaceProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpineInterfaceProfilesSelectorsPortBlocksActive = activeKeys[item.id] === 'fpspineinterfaceprofilesselectorsportblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspineinterfaceprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="fpspineinterfaceprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_spine_interface_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspineinterfaceprofilesselectorsportblocks" title="Port Blocks">
                        {isFpSpineInterfaceProfilesSelectorsPortBlocksActive && (  
                            <FpSpineInterfaceProfilesSelectorsPortBlocks
                              key={item.id}
                              control={control}
...
export default FpSpineInterfaceProfilesSelectors;

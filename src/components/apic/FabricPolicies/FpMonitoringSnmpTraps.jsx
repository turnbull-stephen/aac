...
import FpMonitoringSnmpTrapsDestinations from './FpMonitoringSnmpTrapsDestinations'


const FpMonitoringSnmpTraps = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpMonitoringSnmpTrapsDestinationsActive = activeKeys[item.id] === 'fpmonitoringsnmptrapsdestinations';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpmonitoringsnmptraps'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpmonitoringsnmptraps" title="Snmp Traps">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_monitoring_snmp_traps', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpmonitoringsnmptrapsdestinations" title="Destinations">
                        {isFpMonitoringSnmpTrapsDestinationsActive && (  
                            <FpMonitoringSnmpTrapsDestinations
                              key={item.id}
                              control={control}
...
export default FpMonitoringSnmpTraps;

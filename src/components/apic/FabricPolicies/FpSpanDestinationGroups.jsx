...
import FpSpanDestinationGroupsExpectedState from './FpSpanDestinationGroupsExpectedState'


const FpSpanDestinationGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpanDestinationGroupsExpectedStateActive = activeKeys[item.id] === 'fpspandestinationgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspandestinationgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpspandestinationgroups" title="Destination Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_span_destination_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspandestinationgroupsexpectedstate" title="Expected State">
                        {isFpSpanDestinationGroupsExpectedStateActive && (  
                            <FpSpanDestinationGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default FpSpanDestinationGroups;

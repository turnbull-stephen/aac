...
import FpMonitoringSyslogsDestinations from './FpMonitoringSyslogsDestinations'


const FpMonitoringSyslogs = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpMonitoringSyslogsDestinationsActive = activeKeys[item.id] === 'fpmonitoringsyslogsdestinations';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpmonitoringsyslogs'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpmonitoringsyslogs" title="Syslogs">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_monitoring_syslogs', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpmonitoringsyslogsdestinations" title="Destinations">
                        {isFpMonitoringSyslogsDestinationsActive && (  
                            <FpMonitoringSyslogsDestinations
                              key={item.id}
                              control={control}
...
export default FpMonitoringSyslogs;

...
const  FpRogueEpControl = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fprogueepcontrol'); // Keep state for the Tab selection
  ...
  const hasFpRogueEpControlErrors = Boolean(errors && errors[name] && errors[name].rogue_ep_control)//;
  
  const isFpRogueEpControlActive = key === 'fprogueepcontrol';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fprogueepcontrol" title={tabTitle('Rogue Ep Control', hasFpRogueEpControlErrors)}>
          {isFpRogueEpControlActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_rogue_ep_control', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpRogueEpControl;

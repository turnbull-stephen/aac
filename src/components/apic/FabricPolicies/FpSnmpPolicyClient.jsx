...
import FpSnmpPolicyClientEntry from './FpSnmpPolicyClientEntry'


const FpSnmpPolicyClient = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSnmpPolicyClientEntryActive = activeKeys[item.id] === 'fpsnmppolicycliententry';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpsnmppolicyclient'}
                    ...
                    >
                      <Tab eventKey="fpsnmppolicyclient" title="Clients">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_snmp_policy_client', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpsnmppolicycliententry" title="Entries">
                        {isFpSnmpPolicyClientEntryActive && (  
                            <FpSnmpPolicyClientEntry
                              key={item.id}
                              control={control}
...
export default FpSnmpPolicyClient;

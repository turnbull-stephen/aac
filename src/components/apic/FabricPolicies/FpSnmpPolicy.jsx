...
import FpSnmpPolicyUser from './FpSnmpPolicyUser'
import FpSnmpPolicyTrapForwarder from './FpSnmpPolicyTrapForwarder'
import FpSnmpPolicyClient from './FpSnmpPolicyClient'


const FpSnmpPolicy = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSnmpPolicyUserActive = activeKeys[item.id] === 'fpsnmppolicyuser';
          const isFpSnmpPolicyTrapForwarderActive = activeKeys[item.id] === 'fpsnmppolicytrapforwarder';
          const isFpSnmpPolicyClientActive = activeKeys[item.id] === 'fpsnmppolicyclient';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpsnmppolicy'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpsnmppolicy" title="Snmp Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_snmp_policy', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpsnmppolicyuser" title="Users">
                        {isFpSnmpPolicyUserActive && (  
                            <FpSnmpPolicyUser
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].users`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="fpsnmppolicytrapforwarder" title="Trap Forwarders">
                        {isFpSnmpPolicyTrapForwarderActive && (  
                            <FpSnmpPolicyTrapForwarder
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].trap_forwarders`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="fpsnmppolicyclient" title="Clients">
                        {isFpSnmpPolicyClientActive && (  
                            <FpSnmpPolicyClient
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].clients`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

...
import FpAaaLoginDomainsTacacsProviders from './FpAaaLoginDomainsTacacsProviders'
import FpAaaLoginDomainsLdapProviders from './FpAaaLoginDomainsLdapProviders'
import FpAaaLoginDomainsRadiusProviders from './FpAaaLoginDomainsRadiusProviders'


const FpAaaLoginDomains = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaaLoginDomainsTacacsProvidersActive = activeKeys[item.id] === 'fpaaalogindomainstacacsproviders';
          const isFpAaaLoginDomainsLdapProvidersActive = activeKeys[item.id] === 'fpaaalogindomainsldapproviders';
          const isFpAaaLoginDomainsRadiusProvidersActive = activeKeys[item.id] === 'fpaaalogindomainsradiusproviders';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaalogindomains'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaalogindomains" title="Login Domains">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aaa_login_domains', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaalogindomainstacacsproviders" title="Tacacs Providers">
                        {isFpAaaLoginDomainsTacacsProvidersActive && (  
                            <FpAaaLoginDomainsTacacsProviders
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].tacacs_providers`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="fpaaalogindomainsldapproviders" title="Ldap Providers">
                        {isFpAaaLoginDomainsLdapProvidersActive && (  
                            <FpAaaLoginDomainsLdapProviders
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].ldap_providers`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="fpaaalogindomainsradiusproviders" title="Radius Providers">
                        {isFpAaaLoginDomainsRadiusProvidersActive && (  
                            <FpAaaLoginDomainsRadiusProviders
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].radius_providers`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpAaaLoginDomains;

...
import FpPodProfilesSelectorsPodBlocks from './FpPodProfilesSelectorsPodBlocks'


const FpPodProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpPodProfilesSelectorsPodBlocksActive = activeKeys[item.id] === 'fppodprofilesselectorspodblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fppodprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="fppodprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_pod_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fppodprofilesselectorspodblocks" title="Pod Blocks">
                        {isFpPodProfilesSelectorsPodBlocksActive && (  
                            <FpPodProfilesSelectorsPodBlocks
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].pod_blocks`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpPodProfilesSelectors;

...
import FpSchedulersRecurringWindows from './FpSchedulersRecurringWindows'


const FpSchedulers = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSchedulersRecurringWindowsActive = activeKeys[item.id] === 'fpschedulersrecurringwindows';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpschedulers'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpschedulers" title="Schedulers">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_schedulers', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpschedulersrecurringwindows" title="Recurring Windows">
                        {isFpSchedulersRecurringWindowsActive && (  
                            <FpSchedulersRecurringWindows
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].recurring_windows`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

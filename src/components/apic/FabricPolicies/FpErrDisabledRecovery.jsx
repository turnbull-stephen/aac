...
const  FpErrDisabledRecovery = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fperrdisabledrecovery'); // Keep state for the Tab selection
  ...
  const hasFpErrDisabledRecoveryErrors = Boolean(errors && errors[name] && errors[name].err_disabled_recovery)//;
  
  const isFpErrDisabledRecoveryActive = key === 'fperrdisabledrecovery';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fperrdisabledrecovery" title={tabTitle('Err Disabled Recovery', hasFpErrDisabledRecoveryErrors)}>
          {isFpErrDisabledRecoveryActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_err_disabled_recovery', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpErrDisabledRecovery;

...
import FpDnsPolicyProvider from './FpDnsPolicyProvider'
import FpDnsPolicyDomain from './FpDnsPolicyDomain'


const FpDnsPolicy = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpDnsPolicyProviderActive = activeKeys[item.id] === 'fpdnspolicyprovider';
          const isFpDnsPolicyDomainActive = activeKeys[item.id] === 'fpdnspolicydomain';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpdnspolicy'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpdnspolicy" title="Dns Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_dns_policy', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpdnspolicyprovider" title="Providers">
                        {isFpDnsPolicyProviderActive && (  
                            <FpDnsPolicyProvider
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].providers`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="fpdnspolicydomain" title="Domains">
                        {isFpDnsPolicyDomainActive && (  
                            <FpDnsPolicyDomain
                              key={item.id}
                              control={control}

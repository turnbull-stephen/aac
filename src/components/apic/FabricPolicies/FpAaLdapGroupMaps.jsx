...
import FpAaLdapGroupMapsRules from './FpAaLdapGroupMapsRules'


const FpAaLdapGroupMaps = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpAaLdapGroupMapsRulesActive = activeKeys[item.id] === 'fpaaldapgroupmapsrules';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpaaldapgroupmaps'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpaaldapgroupmaps" title="Group Maps">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_aa_ldap_group_maps', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpaaldapgroupmapsrules" title="Rules">
                        {isFpAaLdapGroupMapsRulesActive && (  
                            <FpAaLdapGroupMapsRules
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].rules`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default FpAaLdapGroupMaps;

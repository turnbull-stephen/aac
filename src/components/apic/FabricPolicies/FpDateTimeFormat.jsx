...
const  FpDateTimeFormat = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('fpdatetimeformat'); // Keep state for the Tab selection
  ...
  const hasFpDateTimeFormatErrors = Boolean(errors && errors[name] && errors[name].date_time_format)//;
  
  const isFpDateTimeFormatActive = key === 'fpdatetimeformat';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        ...
        <Tab eventKey="fpdatetimeformat" title={tabTitle('Date Time Format', hasFpDateTimeFormatErrors)}>
          {isFpDateTimeFormatActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'fabric_policies.fp_date_time_format', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default FpDateTimeFormat;

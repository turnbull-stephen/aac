...
import FpMonitoringCallhomesDestinations from './FpMonitoringCallhomesDestinations'


const FpMonitoringCallhomes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpMonitoringCallhomesDestinationsActive = activeKeys[item.id] === 'fpmonitoringcallhomesdestinations';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpmonitoringcallhomes'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="fpmonitoringcallhomes" title="Callhomes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_monitoring_callhomes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpmonitoringcallhomesdestinations" title="Destinations">
                        {isFpMonitoringCallhomesDestinationsActive && (  
                            <FpMonitoringCallhomesDestinations
                              key={item.id}
                              control={control}
...
export default FpMonitoringCallhomes;

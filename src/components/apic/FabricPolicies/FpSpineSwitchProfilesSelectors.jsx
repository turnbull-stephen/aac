...
import FpSpineSwitchProfilesSelectorsNodeBlocks from './FpSpineSwitchProfilesSelectorsNodeBlocks'


const FpSpineSwitchProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isFpSpineSwitchProfilesSelectorsNodeBlocksActive = activeKeys[item.id] === 'fpspineswitchprofilesselectorsnodeblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'fpspineswitchprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="fpspineswitchprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'fabric_policies.fp_spine_switch_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="fpspineswitchprofilesselectorsnodeblocks" title="Node Blocks">
                        {isFpSpineSwitchProfilesSelectorsNodeBlocksActive && (  
                            <FpSpineSwitchProfilesSelectorsNodeBlocks
                              key={item.id}
                              control={control}
...
export default FpSpineSwitchProfilesSelectors;

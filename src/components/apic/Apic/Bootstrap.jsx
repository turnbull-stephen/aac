...
const  Bootstrap = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('bootstrap'); // Keep state for the Tab selection
  ...
  const hasBootstrapErrors = Boolean(errors && errors[name] && errors[name].bootstrap)//;
  
  const isBootstrapActive = key === 'bootstrap';
  return (
    <div>
            ...
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'apic.bootstrap', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

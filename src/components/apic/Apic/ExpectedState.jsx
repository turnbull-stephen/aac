...
const  ExpectedState = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('expectedstate'); // Keep state for the Tab selection
  ...
  const hasExpectedStateErrors = Boolean(errors && errors[name] && errors[name].expected_state)//;
  
  const isExpectedStateActive = key === 'expectedstate';
  return (
    <div>

...
import ApLeafInterfaceProfilesSelectorsPortBlocks from './ApLeafInterfaceProfilesSelectorsPortBlocks'
import ApLeafInterfaceProfilesSelectorsSubPortBlocks from './ApLeafInterfaceProfilesSelectorsSubPortBlocks'


const ApLeafInterfaceProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApLeafInterfaceProfilesSelectorsPortBlocksActive = activeKeys[item.id] === 'apleafinterfaceprofilesselectorsportblocks';
          const isApLeafInterfaceProfilesSelectorsSubPortBlocksActive = activeKeys[item.id] === 'apleafinterfaceprofilesselectorssubportblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apleafinterfaceprofilesselectors'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apleafinterfaceprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_leaf_interface_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apleafinterfaceprofilesselectorsportblocks" title="Port Blocks">
                        {isApLeafInterfaceProfilesSelectorsPortBlocksActive && (  
                            <ApLeafInterfaceProfilesSelectorsPortBlocks
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].port_blocks`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="apleafinterfaceprofilesselectorssubportblocks" title="Sub Port Blocks">
                        {isApLeafInterfaceProfilesSelectorsSubPortBlocksActive && (  
                            <ApLeafInterfaceProfilesSelectorsSubPortBlocks
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].sub_port_blocks`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default ApLeafInterfaceProfilesSelectors;

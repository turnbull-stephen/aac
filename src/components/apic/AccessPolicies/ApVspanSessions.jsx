...
import ApVspanSessionsSources from './ApVspanSessionsSources'
import ApVspanSessionsDestination from './ApVspanSessionsDestination'
import ApVspanSessionsExpectedState from './ApVspanSessionsExpectedState'


const ApVspanSessions = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApVspanSessionsSourcesActive = activeKeys[item.id] === 'apvspansessionssources';
          const isApVspanSessionsDestinationActive = activeKeys[item.id] === 'apvspansessionsdestination';
          const isApVspanSessionsExpectedStateActive = activeKeys[item.id] === 'apvspansessionsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apvspansessions'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apvspansessions" title="Sessions">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_vspan_sessions', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apvspansessionssources" title="Sources">
                        {isApVspanSessionsSourcesActive && (  
                            <ApVspanSessionsSources
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apvspansessionsdestination" title="Destination">
                        {isApVspanSessionsDestinationActive && (  
                            <ApVspanSessionsDestination
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apvspansessionsexpectedstate" title="Expected State">
                        {isApVspanSessionsExpectedStateActive && (  
                            <ApVspanSessionsExpectedState
                              key={item.id}
                              control={control}
...
export default ApVspanSessions;

...
import ApMstPolicyInstance from './ApMstPolicyInstance'


const ApMstPolicy = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApMstPolicyInstanceActive = activeKeys[item.id] === 'apmstpolicyinstance';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apmstpolicy'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apmstpolicy" title="Mst Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_mst_policy', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isApMstPolicyInstanceActive && (  
                            <ApMstPolicyInstance
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].instances`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

...
import ApSpanSourceGroupsSources from './ApSpanSourceGroupsSources'
import ApSpanSourceGroupsDestination from './ApSpanSourceGroupsDestination'
import ApSpanSourceGroupsExpectedState from './ApSpanSourceGroupsExpectedState'


const ApSpanSourceGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpanSourceGroupsSourcesActive = activeKeys[item.id] === 'apspansourcegroupssources';
          const isApSpanSourceGroupsDestinationActive = activeKeys[item.id] === 'apspansourcegroupsdestination';
          const isApSpanSourceGroupsExpectedStateActive = activeKeys[item.id] === 'apspansourcegroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspansourcegroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apspansourcegroups" title="Source Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_span_source_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspansourcegroupssources" title="Sources">
                        {isApSpanSourceGroupsSourcesActive && (  
                            <ApSpanSourceGroupsSources
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].sources`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="apspansourcegroupsdestination" title="Destination">
                        {isApSpanSourceGroupsDestinationActive && (  
                            <ApSpanSourceGroupsDestination
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].destination`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="apspansourcegroupsexpectedstate" title="Expected State">
                        {isApSpanSourceGroupsExpectedStateActive && (  
                            <ApSpanSourceGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default ApSpanSourceGroups;

...
import ApSpanSourceGroupsSourcesAccessPaths from './ApSpanSourceGroupsSourcesAccessPaths'


const ApSpanSourceGroupsSources = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpanSourceGroupsSourcesAccessPathsActive = activeKeys[item.id] === 'apspansourcegroupssourcesaccesspaths';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspansourcegroupssources'}
                    ...
                    >
                      <Tab eventKey="apspansourcegroupssources" title="Sources">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_span_source_groups_sources', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspansourcegroupssourcesaccesspaths" title="Access Paths">
                        {isApSpanSourceGroupsSourcesAccessPathsActive && (  
                            <ApSpanSourceGroupsSourcesAccessPaths
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].access_paths`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default ApSpanSourceGroupsSources;

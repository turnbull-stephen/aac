...
import ApPhysicalDomainExpectedState from './ApPhysicalDomainExpectedState'


const ApPhysicalDomain = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApPhysicalDomainExpectedStateActive = activeKeys[item.id] === 'apphysicaldomainexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apphysicaldomain'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apphysicaldomain" title="Physical Domains">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_physical_domain', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apphysicaldomainexpectedstate" title="Expected State">
                        {isApPhysicalDomainExpectedStateActive && (  
                            <ApPhysicalDomainExpectedState
                              key={item.id}
                              control={control}
...
export default ApPhysicalDomain;

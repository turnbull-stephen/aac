...
import ApVspanDestinationGroupsDestinations from './ApVspanDestinationGroupsDestinations'
import ApVspanDestinationGroupsExpectedState from './ApVspanDestinationGroupsExpectedState'


const ApVspanDestinationGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Destination Groups'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApVspanDestinationGroupsDestinationsActive = activeKeys[item.id] === 'apvspandestinationgroupsdestinations';
          const isApVspanDestinationGroupsExpectedStateActive = activeKeys[item.id] === 'apvspandestinationgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apvspandestinationgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apvspandestinationgroups" title="Destination Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_vspan_destination_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apvspandestinationgroupsdestinations" title="Destinations">
                        {isApVspanDestinationGroupsDestinationsActive && (  
                            <ApVspanDestinationGroupsDestinations
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].destinations`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="apvspandestinationgroupsexpectedstate" title="Expected State">
                        {isApVspanDestinationGroupsExpectedStateActive && (  
                            <ApVspanDestinationGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default ApVspanDestinationGroups;

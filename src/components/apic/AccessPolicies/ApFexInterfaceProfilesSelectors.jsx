...
import ApFexInterfaceProfilesSelectorsPortBlocks from './ApFexInterfaceProfilesSelectorsPortBlocks'


const ApFexInterfaceProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApFexInterfaceProfilesSelectorsPortBlocksActive = activeKeys[item.id] === 'apfexinterfaceprofilesselectorsportblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apfexinterfaceprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="apfexinterfaceprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_fex_interface_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apfexinterfaceprofilesselectorsportblocks" title="Port Blocks">
                        {isApFexInterfaceProfilesSelectorsPortBlocksActive && (  
                            <ApFexInterfaceProfilesSelectorsPortBlocks
                              key={item.id}
                              control={control}
...
export default ApFexInterfaceProfilesSelectors;

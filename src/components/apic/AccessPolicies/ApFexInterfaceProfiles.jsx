...
import ApFexInterfaceProfilesSelectors from './ApFexInterfaceProfilesSelectors'
import ApFexInterfaceProfilesExpectedState from './ApFexInterfaceProfilesExpectedState'


const ApFexInterfaceProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Fex Interface Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApFexInterfaceProfilesSelectorsActive = activeKeys[item.id] === 'apfexinterfaceprofilesselectors';
          const isApFexInterfaceProfilesExpectedStateActive = activeKeys[item.id] === 'apfexinterfaceprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apfexinterfaceprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apfexinterfaceprofiles" title="Fex Interface Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_fex_interface_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apfexinterfaceprofilesselectors" title="Selectors">
                        {isApFexInterfaceProfilesSelectorsActive && (  
                            <ApFexInterfaceProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apfexinterfaceprofilesexpectedstate" title="Expected State">
                        {isApFexInterfaceProfilesExpectedStateActive && (  
                            <ApFexInterfaceProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default ApFexInterfaceProfiles;

...
import ApLeafSwitchProfilesSelectors from './ApLeafSwitchProfilesSelectors'
import ApLeafSwitchProfilesExpectedState from './ApLeafSwitchProfilesExpectedState'


const ApLeafSwitchProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApLeafSwitchProfilesSelectorsActive = activeKeys[item.id] === 'apleafswitchprofilesselectors';
          const isApLeafSwitchProfilesExpectedStateActive = activeKeys[item.id] === 'apleafswitchprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apleafswitchprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apleafswitchprofiles" title="Leaf Switch Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_leaf_switch_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apleafswitchprofilesselectors" title="Selectors">
                        {isApLeafSwitchProfilesSelectorsActive && (  
                            <ApLeafSwitchProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apleafswitchprofilesexpectedstate" title="Expected State">
                        {isApLeafSwitchProfilesExpectedStateActive && (  
                            <ApLeafSwitchProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default ApLeafSwitchProfiles;

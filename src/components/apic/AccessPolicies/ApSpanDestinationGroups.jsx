...
import ApSpanDestinationGroupsExpectedState from './ApSpanDestinationGroupsExpectedState'


const ApSpanDestinationGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpanDestinationGroupsExpectedStateActive = activeKeys[item.id] === 'apspandestinationgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspandestinationgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apspandestinationgroups" title="Destination Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_span_destination_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspandestinationgroupsexpectedstate" title="Expected State">
                        {isApSpanDestinationGroupsExpectedStateActive && (  
                            <ApSpanDestinationGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default ApSpanDestinationGroups;

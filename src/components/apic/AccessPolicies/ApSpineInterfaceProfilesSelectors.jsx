...
import ApSpineInterfaceProfilesSelectorsPortBlocks from './ApSpineInterfaceProfilesSelectorsPortBlocks'


const ApSpineInterfaceProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpineInterfaceProfilesSelectorsPortBlocksActive = activeKeys[item.id] === 'apspineinterfaceprofilesselectorsportblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspineinterfaceprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="apspineinterfaceprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_spine_interface_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspineinterfaceprofilesselectorsportblocks" title="Port Blocks">
                        {isApSpineInterfaceProfilesSelectorsPortBlocksActive && (  
                            <ApSpineInterfaceProfilesSelectorsPortBlocks
                              key={item.id}
                              control={control}
...
export default ApSpineInterfaceProfilesSelectors;

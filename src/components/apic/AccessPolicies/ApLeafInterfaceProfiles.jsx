...
import ApLeafInterfaceProfilesSelectors from './ApLeafInterfaceProfilesSelectors'
import ApLeafInterfaceProfilesExpectedState from './ApLeafInterfaceProfilesExpectedState'


const ApLeafInterfaceProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Leaf Interface Profiles'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApLeafInterfaceProfilesSelectorsActive = activeKeys[item.id] === 'apleafinterfaceprofilesselectors';
          const isApLeafInterfaceProfilesExpectedStateActive = activeKeys[item.id] === 'apleafinterfaceprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apleafinterfaceprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apleafinterfaceprofiles" title="Leaf Interface Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_leaf_interface_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isApLeafInterfaceProfilesSelectorsActive && (  
                            <ApLeafInterfaceProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apleafinterfaceprofilesexpectedstate" title="Expected State">
                        {isApLeafInterfaceProfilesExpectedStateActive && (  
                            <ApLeafInterfaceProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default ApLeafInterfaceProfiles;

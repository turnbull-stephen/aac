...
import ApSpineSwitchProfilesSelectors from './ApSpineSwitchProfilesSelectors'
import ApSpineSwitchProfilesExpectedState from './ApSpineSwitchProfilesExpectedState'


const ApSpineSwitchProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApSpineSwitchProfilesSelectorsActive = activeKeys[item.id] === 'apspineswitchprofilesselectors';
          const isApSpineSwitchProfilesExpectedStateActive = activeKeys[item.id] === 'apspineswitchprofilesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apspineswitchprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apspineswitchprofiles" title="Spine Switch Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_spine_switch_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apspineswitchprofilesselectors" title="Selectors">
                        {isApSpineSwitchProfilesSelectorsActive && (  
                            <ApSpineSwitchProfilesSelectors
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apspineswitchprofilesexpectedstate" title="Expected State">
                        {isApSpineSwitchProfilesExpectedStateActive && (  
                            <ApSpineSwitchProfilesExpectedState
                              key={item.id}
                              control={control}
...
export default ApSpineSwitchProfiles;

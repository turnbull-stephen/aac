...
import ApLeafSwitchProfilesSelectorsNodeBlocks from './ApLeafSwitchProfilesSelectorsNodeBlocks'


const ApLeafSwitchProfilesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApLeafSwitchProfilesSelectorsNodeBlocksActive = activeKeys[item.id] === 'apleafswitchprofilesselectorsnodeblocks';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apleafswitchprofilesselectors'}
                    ...
                    >
                      <Tab eventKey="apleafswitchprofilesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_leaf_switch_profiles_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apleafswitchprofilesselectorsnodeblocks" title="Node Blocks">
                        {isApLeafSwitchProfilesSelectorsNodeBlocksActive && (  
                            <ApLeafSwitchProfilesSelectorsNodeBlocks
                              key={item.id}
                              control={control}
...
export default ApLeafSwitchProfilesSelectors;

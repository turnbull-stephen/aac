...
import ApVlanPoolRange from './ApVlanPoolRange'


const ApVlanPool = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApVlanPoolRangeActive = activeKeys[item.id] === 'apvlanpoolrange';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apvlanpool'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apvlanpool" title="Vlan Pools">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_vlan_pool', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apvlanpoolrange" title="Ranges">
                        {isApVlanPoolRangeActive && (  
                            <ApVlanPoolRange
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].ranges`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

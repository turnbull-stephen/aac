...
import ApAaepEndpointGroups from './ApAaepEndpointGroups'
import ApAaepExpectedState from './ApAaepExpectedState'


const ApAaep = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isApAaepEndpointGroupsActive = activeKeys[item.id] === 'apaaependpointgroups';
          const isApAaepExpectedStateActive = activeKeys[item.id] === 'apaaepexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'apaaep'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="apaaep" title="Aaeps">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'access_policies.ap_aaep', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="apaaependpointgroups" title="Endpoint Groups">
                        {isApAaepEndpointGroupsActive && (  
                            <ApAaepEndpointGroups
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="apaaepexpectedstate" title="Expected State">
                        {isApAaepExpectedStateActive && (  
                            <ApAaepExpectedState
                              key={item.id}
                              control={control}

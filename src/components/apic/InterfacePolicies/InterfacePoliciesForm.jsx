...
const InterfacePoliciesForm = () => {
      ...
      if (fileExtension === 'yaml' || fileExtension === 'yml') {
        ...
        reader.onload = (event) => {
          try {
            ...
            reset(loadedData.apic.interface_policies);
            onSubmit(loadedData.apic.interface_policies)
  ...
  // Define the Root for apic/interface_policies
  
  const root = {
    "apic": { "interface_policiess": {} }
  }
  ...
  const onSubmit = (data) => {
    ...
    root.apic.interface_policiess = processedData;
    
    // To display the YAML representation of the interface_policies array,
  ...
  const [key, setKey] = useState('ipnode'); // Keep state for the Tab selection
  


  const hasIpNodeErrors = Boolean(errors.nodes);
    ...
    <div className="full-height-layout">
      <Container fluid>
        <Row>
          <Col md={3}>
            <div>
              <label>Load Interface Policies Data:</label>
          ...
          <Col md={9}>
            <div >
            <h3 className='mt-2'>Interface Policies</h3>
                  ...
                  <Card>
                    <Card.Body>
                      <Tabs
                        ...
                        //fill
                      >
                        <Tab eventKey="ipnode" title={tabTitle('Nodes', hasIpNodeErrors)}>
                          <Card>
                            <Card.Body>
                            <IpNode control={control} register={register} errors={errors} clearErrors={clearErrors} setError={setError} name={'nodes'} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        </Tabs>
...
export default InterfacePoliciesForm;

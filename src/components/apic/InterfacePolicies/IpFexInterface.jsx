...
import IpFexInterfaceExpectedState from './IpFexInterfaceExpectedState'


const IpFexInterface = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isIpFexInterfaceExpectedStateActive = activeKeys[item.id] === 'ipfexinterfaceexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'ipfexinterface'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="ipfexinterface" title="Interfaces">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'interface_policies.ip_fex_interface', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="ipfexinterfaceexpectedstate" title="Expected State">
                        {isIpFexInterfaceExpectedStateActive && (  
                            <IpFexInterfaceExpectedState
                              key={item.id}
                              control={control}

...
import IpInterfaceSubPorts from './IpInterfaceSubPorts'
import IpInterfaceExpectedState from './IpInterfaceExpectedState'


const IpInterface = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isIpInterfaceSubPortsActive = activeKeys[item.id] === 'ipinterfacesubports';
          const isIpInterfaceExpectedStateActive = activeKeys[item.id] === 'ipinterfaceexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'ipinterface'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="ipinterface" title="Interfaces">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'interface_policies.ip_interface', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="ipinterfacesubports" title="Sub Ports">
                        {isIpInterfaceSubPortsActive && (  
                            <IpInterfaceSubPorts
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].sub_ports`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="ipinterfaceexpectedstate" title="Expected State">
                        {isIpInterfaceExpectedStateActive && (  
                            <IpInterfaceExpectedState
                              key={item.id}
                              control={control}

...
import TenMulticastRouteMapEntries from './TenMulticastRouteMapEntries'
import TenMulticastRouteMapsExpectedState from './TenMulticastRouteMapsExpectedState'


const TenMulticastRouteMaps = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Multicast Route Maps'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenMulticastRouteMapEntriesActive = activeKeys[item.id] === 'tenmulticastroutemapentries';
          const isTenMulticastRouteMapsExpectedStateActive = activeKeys[item.id] === 'tenmulticastroutemapsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenmulticastroutemaps'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenmulticastroutemaps" title="Multicast Route Maps">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_multicast_route_maps', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenmulticastroutemapentries" title="Entries">
                        {isTenMulticastRouteMapEntriesActive && (  
                            <TenMulticastRouteMapEntries
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].entries`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenmulticastroutemapsexpectedstate" title="Expected State">
                        {isTenMulticastRouteMapsExpectedStateActive && (  
                            <TenMulticastRouteMapsExpectedState
                              key={item.id}
                              control={control}
...
export default TenMulticastRouteMaps;

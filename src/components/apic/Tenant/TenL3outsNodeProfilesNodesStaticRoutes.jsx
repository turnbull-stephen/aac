...
import TenL3outsNodeProfilesNodesStaticRoutesNextHops from './TenL3outsNodeProfilesNodesStaticRoutesNextHops'


const TenL3outsNodeProfilesNodesStaticRoutes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodeProfilesNodesStaticRoutesNextHopsActive = activeKeys[item.id] === 'tenl3outsnodeprofilesnodesstaticroutesnexthops';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodeprofilesnodesstaticroutes'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenl3outsnodeprofilesnodesstaticroutes" title="Static Routes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_node_profiles_nodes_static_routes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsnodeprofilesnodesstaticroutesnexthops" title="Next Hops">
                        {isTenL3outsNodeProfilesNodesStaticRoutesNextHopsActive && (  
                            <TenL3outsNodeProfilesNodesStaticRoutesNextHops
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].next_hops`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenL3outsNodeProfilesNodesStaticRoutes;

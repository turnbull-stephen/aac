...
import TenOobContractsSubjects from './TenOobContractsSubjects'
import TenOobContractsExpectedState from './TenOobContractsExpectedState'


const TenOobContracts = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenOobContractsSubjectsActive = activeKeys[item.id] === 'tenoobcontractssubjects';
          const isTenOobContractsExpectedStateActive = activeKeys[item.id] === 'tenoobcontractsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenoobcontracts'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenoobcontracts" title="Oob Contracts">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_oob_contracts', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenoobcontractssubjects" title="Subjects">
                        {isTenOobContractsSubjectsActive && (  
                            <TenOobContractsSubjects
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].subjects`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenoobcontractsexpectedstate" title="Expected State">
                        {isTenOobContractsExpectedStateActive && (  
                            <TenOobContractsExpectedState
                              key={item.id}
                              control={control}
...
export default TenOobContracts;

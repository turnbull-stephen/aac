...
import TenDhcpProvider from './TenDhcpProvider'


const TenDhcpRelayPolicy = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Dhcp Relay Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenDhcpProviderActive = activeKeys[item.id] === 'tendhcpprovider';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tendhcprelaypolicy'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tendhcprelaypolicy" title="Dhcp Relay Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_dhcp_relay_policy', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tendhcpprovider" title="Providers">
                        {isTenDhcpProviderActive && (  
                            <TenDhcpProvider
                              key={item.id}
                              control={control}
...
export default TenDhcpRelayPolicy;

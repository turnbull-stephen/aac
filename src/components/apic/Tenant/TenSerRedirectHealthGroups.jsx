...
import TenSerRedirectHealthGroupsExpectedState from './TenSerRedirectHealthGroupsExpectedState'


const TenSerRedirectHealthGroups = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Redirect Health Groups'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerRedirectHealthGroupsExpectedStateActive = activeKeys[item.id] === 'tenserredirecthealthgroupsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserredirecthealthgroups'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenserredirecthealthgroups" title="Redirect Health Groups">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_redirect_health_groups', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserredirecthealthgroupsexpectedstate" title="Expected State">
                        {isTenSerRedirectHealthGroupsExpectedStateActive && (  
                            <TenSerRedirectHealthGroupsExpectedState
                              key={item.id}
                              control={control}
...
export default TenSerRedirectHealthGroups;

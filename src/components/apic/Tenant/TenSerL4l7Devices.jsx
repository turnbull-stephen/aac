...
import TenSerL4l7DevicesConcreteDevices from './TenSerL4l7DevicesConcreteDevices'
import TenSerL4l7DevicesLogicalInterfaces from './TenSerL4l7DevicesLogicalInterfaces'
import TenSerL4l7DevicesExpectedState from './TenSerL4l7DevicesExpectedState'


const TenSerL4l7Devices = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerL4l7DevicesConcreteDevicesActive = activeKeys[item.id] === 'tenserl4l7devicesconcretedevices';
          const isTenSerL4l7DevicesLogicalInterfacesActive = activeKeys[item.id] === 'tenserl4l7deviceslogicalinterfaces';
          const isTenSerL4l7DevicesExpectedStateActive = activeKeys[item.id] === 'tenserl4l7devicesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserl4l7devices'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenserl4l7devices" title="L4l7 Devices">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_l4l7_devices', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserl4l7devicesconcretedevices" title="Concrete Devices">
                        {isTenSerL4l7DevicesConcreteDevicesActive && (  
                            <TenSerL4l7DevicesConcreteDevices
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].concrete_devices`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserl4l7deviceslogicalinterfaces" title="Logical Interfaces">
                        {isTenSerL4l7DevicesLogicalInterfacesActive && (  
                            <TenSerL4l7DevicesLogicalInterfaces
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].logical_interfaces`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserl4l7devicesexpectedstate" title="Expected State">
                        {isTenSerL4l7DevicesExpectedStateActive && (  
                            <TenSerL4l7DevicesExpectedState
                              key={item.id}
                              control={control}
...
export default TenSerL4l7Devices;

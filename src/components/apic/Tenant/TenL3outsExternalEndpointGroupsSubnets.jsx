...
import TenL3outsExternalEndpointGroupsSubnetsRouteControlProfiles from './TenL3outsExternalEndpointGroupsSubnetsRouteControlProfiles'


const TenL3outsExternalEndpointGroupsSubnets = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsExternalEndpointGroupsSubnetsRouteControlProfilesActive = activeKeys[item.id] === 'tenl3outsexternalendpointgroupssubnetsroutecontrolprofiles';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsexternalendpointgroupssubnets'}
                    ...
                    >
                      <Tab eventKey="tenl3outsexternalendpointgroupssubnets" title="Subnets">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_external_endpoint_groups_subnets', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsexternalendpointgroupssubnetsroutecontrolprofiles" title="Route Control Profiles">
                        {isTenL3outsExternalEndpointGroupsSubnetsRouteControlProfilesActive && (  
                            <TenL3outsExternalEndpointGroupsSubnetsRouteControlProfiles
                              key={item.id}
                              control={control}
...
export default TenL3outsExternalEndpointGroupsSubnets;

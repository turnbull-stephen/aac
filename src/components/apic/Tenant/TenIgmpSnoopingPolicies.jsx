...
import TenIgmpSnoopingPoliciesExpectedState from './TenIgmpSnoopingPoliciesExpectedState'


const TenIgmpSnoopingPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Igmp Snooping Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenIgmpSnoopingPoliciesExpectedStateActive = activeKeys[item.id] === 'tenigmpsnoopingpoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenigmpsnoopingpolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenigmpsnoopingpolicies" title="Igmp Snooping Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_igmp_snooping_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenigmpsnoopingpoliciesexpectedstate" title="Expected State">
                        {isTenIgmpSnoopingPoliciesExpectedStateActive && (  
                            <TenIgmpSnoopingPoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenIgmpSnoopingPolicies;

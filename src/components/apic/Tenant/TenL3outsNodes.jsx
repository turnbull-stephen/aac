...
import TenL3outsNodesStaticRoutes from './TenL3outsNodesStaticRoutes'
import TenL3outsNodesInterfaces from './TenL3outsNodesInterfaces'


const TenL3outsNodes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodesStaticRoutesActive = activeKeys[item.id] === 'tenl3outsnodesstaticroutes';
          const isTenL3outsNodesInterfacesActive = activeKeys[item.id] === 'tenl3outsnodesinterfaces';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodes'}
                    ...
                    >
                      <Tab eventKey="tenl3outsnodes" title="Nodes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_nodes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsnodesstaticroutes" title="Static Routes">
                        {isTenL3outsNodesStaticRoutesActive && (  
                            <TenL3outsNodesStaticRoutes
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].static_routes`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        {isTenL3outsNodesInterfacesActive && (  
                            <TenL3outsNodesInterfaces
                              key={item.id}
                              control={control}

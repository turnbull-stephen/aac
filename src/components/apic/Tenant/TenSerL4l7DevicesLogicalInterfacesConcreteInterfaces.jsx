...
const TenSerL4l7DevicesLogicalInterfacesConcreteInterfaces = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Concrete Interfaces'
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserl4l7deviceslogicalinterfacesconcreteinterfaces'}
                        ...
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_l4l7_devices_logical_interfaces_concrete_interfaces', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
...
export default TenSerL4l7DevicesLogicalInterfacesConcreteInterfaces;

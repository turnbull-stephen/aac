...
const TenBgpPeerPrefixPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Bgp Peer Prefix Policies'
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenbgppeerprefixpolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenbgppeerprefixpolicies" title="Bgp Peer Prefix Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_bgp_peer_prefix_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
...
export default TenBgpPeerPrefixPolicies;

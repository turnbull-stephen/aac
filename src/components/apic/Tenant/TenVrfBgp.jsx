...
const  TenVrfBgp = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors, formData }) => {
   
  const [key, setKey] = useState('tenvrfbgp'); // Keep state for the Tab selection
  ...
  const hasTenVrfBgpErrors = Boolean(errors && errors[name] && errors[name].bgp)//;
  
  const isTenVrfBgpActive = key === 'tenvrfbgp';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        <Tab eventKey="tenvrfbgp" title={tabTitle('Bgp', hasTenVrfBgpErrors)}>
          {isTenVrfBgpActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'tenant.ten_vrf_bgp', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

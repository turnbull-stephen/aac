...
import TenSrMplsL3outsNodeProfilesNodes from './TenSrMplsL3outsNodeProfilesNodes'
import TenSrMplsL3outsNodeProfilesInterfaceProfiles from './TenSrMplsL3outsNodeProfilesInterfaceProfiles'
import TenSrMplsL3outsNodeProfilesEvpnConnectivity from './TenSrMplsL3outsNodeProfilesEvpnConnectivity'


const TenSrMplsL3outsNodeProfiles = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSrMplsL3outsNodeProfilesNodesActive = activeKeys[item.id] === 'tensrmplsl3outsnodeprofilesnodes';
          const isTenSrMplsL3outsNodeProfilesInterfaceProfilesActive = activeKeys[item.id] === 'tensrmplsl3outsnodeprofilesinterfaceprofiles';
          const isTenSrMplsL3outsNodeProfilesEvpnConnectivityActive = activeKeys[item.id] === 'tensrmplsl3outsnodeprofilesevpnconnectivity';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tensrmplsl3outsnodeprofiles'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tensrmplsl3outsnodeprofiles" title="Node Profiles">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_sr_mpls_l3outs_node_profiles', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tensrmplsl3outsnodeprofilesnodes" title="Nodes">
                        {isTenSrMplsL3outsNodeProfilesNodesActive && (  
                            <TenSrMplsL3outsNodeProfilesNodes
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tensrmplsl3outsnodeprofilesinterfaceprofiles" title="Interface Profiles">
                        {isTenSrMplsL3outsNodeProfilesInterfaceProfilesActive && (  
                            <TenSrMplsL3outsNodeProfilesInterfaceProfiles
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].interface_profiles`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tensrmplsl3outsnodeprofilesevpnconnectivity" title="Evpn Connectivity">
                        {isTenSrMplsL3outsNodeProfilesEvpnConnectivityActive && (  
                            <TenSrMplsL3outsNodeProfilesEvpnConnectivity
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].evpn_connectivity`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenSrMplsL3outsNodeProfiles;

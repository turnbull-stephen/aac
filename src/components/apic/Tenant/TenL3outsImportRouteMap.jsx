...
const  TenL3outsImportRouteMap = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors, formData }) => {
   
  const [key, setKey] = useState('tenl3outsimportroutemap'); // Keep state for the Tab selection
  ...
  const hasTenL3outsImportRouteMapErrors = Boolean(errors && errors[name] && errors[name].import_route_map)//;
  const hasTenL3outsRouteMapContextErrors = Boolean(errors && errors[name] && errors[name].contexts);
  
  const isTenL3outsImportRouteMapActive = key === 'tenl3outsimportroutemap';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        <Tab eventKey="tenl3outsimportroutemap" title={tabTitle('Import Route Map', hasTenL3outsImportRouteMapErrors)}>
          {isTenL3outsImportRouteMapActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'tenant.ten_l3outs_import_route_map', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}
...
export default TenL3outsImportRouteMap;

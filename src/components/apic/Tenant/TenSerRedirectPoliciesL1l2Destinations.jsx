...
import TenSerRedirectPoliciesL1l2DestinationsConcreteInterface from './TenSerRedirectPoliciesL1l2DestinationsConcreteInterface'


const TenSerRedirectPoliciesL1l2Destinations = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerRedirectPoliciesL1l2DestinationsConcreteInterfaceActive = activeKeys[item.id] === 'tenserredirectpoliciesl1l2destinationsconcreteinterface';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserredirectpoliciesl1l2destinations'}
                    ...
                    >
                      <Tab eventKey="tenserredirectpoliciesl1l2destinations" title="L1l2 Destinations">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_redirect_policies_l1l2_destinations', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserredirectpoliciesl1l2destinationsconcreteinterface" title="Concrete Interface">
                        {isTenSerRedirectPoliciesL1l2DestinationsConcreteInterfaceActive && (  
                            <TenSerRedirectPoliciesL1l2DestinationsConcreteInterface
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].concrete_interface`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenSerRedirectPoliciesL1l2Destinations;

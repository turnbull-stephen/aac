...
import TenL3outsNodesInterfacesBgpPeers from './TenL3outsNodesInterfacesBgpPeers'
import TenL3outsNodesInterfacesPaths from './TenL3outsNodesInterfacesPaths'
import TenL3outsNodesInterfacesExpectedState from './TenL3outsNodesInterfacesExpectedState'


const TenL3outsNodesInterfaces = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodesInterfacesBgpPeersActive = activeKeys[item.id] === 'tenl3outsnodesinterfacesbgppeers';
          const isTenL3outsNodesInterfacesPathsActive = activeKeys[item.id] === 'tenl3outsnodesinterfacespaths';
          const isTenL3outsNodesInterfacesExpectedStateActive = activeKeys[item.id] === 'tenl3outsnodesinterfacesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodesinterfaces'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenl3outsnodesinterfaces" title="Interfaces">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_nodes_interfaces', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenl3outsnodesinterfacesbgppeers" title="Bgp Peers">
                        {isTenL3outsNodesInterfacesBgpPeersActive && (  
                            <TenL3outsNodesInterfacesBgpPeers
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenl3outsnodesinterfacespaths" title="Paths">
                        {isTenL3outsNodesInterfacesPathsActive && (  
                            <TenL3outsNodesInterfacesPaths
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenl3outsnodesinterfacesexpectedstate" title="Expected State">
                        {isTenL3outsNodesInterfacesExpectedStateActive && (  
                            <TenL3outsNodesInterfacesExpectedState
                              key={item.id}
                              control={control}
...
export default TenL3outsNodesInterfaces;

...
import TenContractsSubjects from './TenContractsSubjects'
import TenContractsExpectedState from './TenContractsExpectedState'


const TenContracts = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenContractsSubjectsActive = activeKeys[item.id] === 'tencontractssubjects';
          const isTenContractsExpectedStateActive = activeKeys[item.id] === 'tencontractsexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tencontracts'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tencontracts" title="Contracts">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_contracts', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tencontractssubjects" title="Subjects">
                        {isTenContractsSubjectsActive && (  
                            <TenContractsSubjects
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tencontractsexpectedstate" title="Expected State">
                        {isTenContractsExpectedStateActive && (  
                            <TenContractsExpectedState
                              key={item.id}
                              control={control}

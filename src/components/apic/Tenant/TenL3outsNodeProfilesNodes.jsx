...
import TenL3outsNodeProfilesNodesStaticRoutes from './TenL3outsNodeProfilesNodesStaticRoutes'


const TenL3outsNodeProfilesNodes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenL3outsNodeProfilesNodesStaticRoutesActive = activeKeys[item.id] === 'tenl3outsnodeprofilesnodesstaticroutes';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenl3outsnodeprofilesnodes'}
                    ...
                    >
                      <Tab eventKey="tenl3outsnodeprofilesnodes" title="Nodes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_l3outs_node_profiles_nodes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isTenL3outsNodeProfilesNodesStaticRoutesActive && (  
                            <TenL3outsNodeProfilesNodesStaticRoutes
                              key={item.id}
                              control={control}
...
export default TenL3outsNodeProfilesNodes;

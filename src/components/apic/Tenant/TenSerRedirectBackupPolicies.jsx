...
import TenSerRedirectBackupPoliciesL3Destinations from './TenSerRedirectBackupPoliciesL3Destinations'
import TenSerRedirectBackupPoliciesExpectedState from './TenSerRedirectBackupPoliciesExpectedState'


const TenSerRedirectBackupPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Redirect Backup Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerRedirectBackupPoliciesL3DestinationsActive = activeKeys[item.id] === 'tenserredirectbackuppoliciesl3destinations';
          const isTenSerRedirectBackupPoliciesExpectedStateActive = activeKeys[item.id] === 'tenserredirectbackuppoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserredirectbackuppolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenserredirectbackuppolicies" title="Redirect Backup Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_redirect_backup_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserredirectbackuppoliciesl3destinations" title="L3 Destinations">
                        {isTenSerRedirectBackupPoliciesL3DestinationsActive && (  
                            <TenSerRedirectBackupPoliciesL3Destinations
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenserredirectbackuppoliciesexpectedstate" title="Expected State">
                        {isTenSerRedirectBackupPoliciesExpectedStateActive && (  
                            <TenSerRedirectBackupPoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenSerRedirectBackupPolicies;

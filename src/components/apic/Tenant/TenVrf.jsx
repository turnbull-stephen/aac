...
import TenVrfLeakedInternalPrefixes from './TenVrfLeakedInternalPrefixes'
import TenVrfLeakedExternalPrefixes from './TenVrfLeakedExternalPrefixes'
import TenVrfContracts from './TenVrfContracts'
import TenVrfBgp from './TenVrfBgp'
import TenVrfOspf from './TenVrfOspf'
import TenVrfPim from './TenVrfPim'
import TenVrfExpectedState from './TenVrfExpectedState'


const TenVrf = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenVrfLeakedInternalPrefixesActive = activeKeys[item.id] === 'tenvrfleakedinternalprefixes';
          const isTenVrfLeakedExternalPrefixesActive = activeKeys[item.id] === 'tenvrfleakedexternalprefixes';
          const isTenVrfContractsActive = activeKeys[item.id] === 'tenvrfcontracts';
          const isTenVrfBgpActive = activeKeys[item.id] === 'tenvrfbgp';
          const isTenVrfOspfActive = activeKeys[item.id] === 'tenvrfospf';
          const isTenVrfPimActive = activeKeys[item.id] === 'tenvrfpim';
          const isTenVrfExpectedStateActive = activeKeys[item.id] === 'tenvrfexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenvrf'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenvrf" title="Vrfs">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_vrf', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenvrfleakedinternalprefixes" title="Leaked Internal Prefixes">
                        {isTenVrfLeakedInternalPrefixesActive && (  
                            <TenVrfLeakedInternalPrefixes
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].leaked_internal_prefixes`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenvrfleakedexternalprefixes" title="Leaked External Prefixes">
                        {isTenVrfLeakedExternalPrefixesActive && (  
                            <TenVrfLeakedExternalPrefixes
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].leaked_external_prefixes`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenvrfcontracts" title="Contracts">
                        {isTenVrfContractsActive && (  
                            <TenVrfContracts
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenvrfbgp" title="Bgp">
                        {isTenVrfBgpActive && (  
                            <TenVrfBgp
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].bgp`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenvrfospf" title="Ospf">
                        {isTenVrfOspfActive && (  
                            <TenVrfOspf
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tenvrfpim" title="Pim">
                        {isTenVrfPimActive && (  
                            <TenVrfPim
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].pim`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenvrfexpectedstate" title="Expected State">
                        {isTenVrfExpectedStateActive && (  
                            <TenVrfExpectedState
                              key={item.id}
                              control={control}

...
import TenSerDeviceSelectionPoliciesConsumerProvider from './TenSerDeviceSelectionPoliciesConsumerProvider'
import TenSerDeviceSelectionPoliciesCopyService from './TenSerDeviceSelectionPoliciesCopyService'
import TenSerDeviceSelectionPoliciesExpectedState from './TenSerDeviceSelectionPoliciesExpectedState'


const TenSerDeviceSelectionPolicies = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Device Selection Policies'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerDeviceSelectionPoliciesConsumerProviderActive = activeKeys[item.id] === 'tenserdeviceselectionpoliciesconsumerprovider';
          const isTenSerDeviceSelectionPoliciesCopyServiceActive = activeKeys[item.id] === 'tenserdeviceselectionpoliciescopyservice';
          const isTenSerDeviceSelectionPoliciesExpectedStateActive = activeKeys[item.id] === 'tenserdeviceselectionpoliciesexpectedstate';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserdeviceselectionpolicies'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tenserdeviceselectionpolicies" title="Device Selection Policies">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_device_selection_policies', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserdeviceselectionpoliciesconsumerprovider" title="Consumer">
                        {isTenSerDeviceSelectionPoliciesConsumerProviderActive && (  
                            <TenSerDeviceSelectionPoliciesConsumerProvider
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].consumer`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserdeviceselectionpoliciesconsumerprovider" title="Provider">
                        {isTenSerDeviceSelectionPoliciesConsumerProviderActive && (  
                            <TenSerDeviceSelectionPoliciesConsumerProvider
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].provider`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserdeviceselectionpoliciescopyservice" title="Copy Service">
                        {isTenSerDeviceSelectionPoliciesCopyServiceActive && (  
                            <TenSerDeviceSelectionPoliciesCopyService
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].copy_service`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenserdeviceselectionpoliciesexpectedstate" title="Expected State">
                        {isTenSerDeviceSelectionPoliciesExpectedStateActive && (  
                            <TenSerDeviceSelectionPoliciesExpectedState
                              key={item.id}
                              control={control}
...
export default TenSerDeviceSelectionPolicies;

...
import TenQosDscpPriorityMap from './TenQosDscpPriorityMap'
import TenQosDot1pClassifiers from './TenQosDot1pClassifiers'


const TenQos = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenQosDscpPriorityMapActive = activeKeys[item.id] === 'tenqosdscpprioritymap';
          const isTenQosDot1pClassifiersActive = activeKeys[item.id] === 'tenqosdot1pclassifiers';
                  ...
                  <Accordion.Body>
                    ...
                    >
                      <Tab eventKey="tenqos" title="Qos">
                        ...
                        <Tab eventKey="tenqosdscpprioritymap" title="Dscp Priority Maps">
                        {isTenQosDscpPriorityMapActive && (  
                            <TenQosDscpPriorityMap
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].dscp_priority_maps`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tenqosdot1pclassifiers" title="Dot1p Classifiers">
                        {isTenQosDot1pClassifiersActive && (  
                            <TenQosDot1pClassifiers
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].dot1p_classifiers`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

...
import TenSerL4l7DevicesLogicalInterfacesConcreteInterfaces from './TenSerL4l7DevicesLogicalInterfacesConcreteInterfaces'


const TenSerL4l7DevicesLogicalInterfaces = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
  ...
  const title = 'Logical Interfaces'
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTenSerL4l7DevicesLogicalInterfacesConcreteInterfacesActive = activeKeys[item.id] === 'tenserl4l7deviceslogicalinterfacesconcreteinterfaces';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tenserl4l7deviceslogicalinterfaces'}
                    ...
                    >
                      <Tab eventKey="tenserl4l7deviceslogicalinterfaces" title="Logical Interfaces">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'tenant.ten_ser_l4l7_devices_logical_interfaces', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tenserl4l7deviceslogicalinterfacesconcreteinterfaces" title="Concrete Interfaces">
                        {isTenSerL4l7DevicesLogicalInterfacesConcreteInterfacesActive && (  
                            <TenSerL4l7DevicesLogicalInterfacesConcreteInterfaces
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].concrete_interfaces`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TenSerL4l7DevicesLogicalInterfaces;

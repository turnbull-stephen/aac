...
import SitesPodsSpinesInterfaces from './SitesPodsSpinesInterfaces'


const SitesPodsSpines = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isSitesPodsSpinesInterfacesActive = activeKeys[item.id] === 'sitespodsspinesinterfaces';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'sitespodsspines'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="sitespodsspines" title="Spines">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.sites_pods_spines', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isSitesPodsSpinesInterfacesActive && (  
                            <SitesPodsSpinesInterfaces
                              key={item.id}
                              control={control}
...
export default SitesPodsSpines;

...
import SitesLocation from './SitesLocation'
import SitesBgp from './SitesBgp'
import SitesOspf from './SitesOspf'
import SitesOspfPolicies from './SitesOspfPolicies'
import SitesPods from './SitesPods'


const Sites = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isSitesLocationActive = activeKeys[item.id] === 'siteslocation';
          const isSitesBgpActive = activeKeys[item.id] === 'sitesbgp';
          const isSitesOspfActive = activeKeys[item.id] === 'sitesospf';
          const isSitesOspfPoliciesActive = activeKeys[item.id] === 'sitesospfpolicies';
          const isSitesPodsActive = activeKeys[item.id] === 'sitespods';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'sites'}
                    ...
                    >
                      <Tab eventKey="sites" title="Sites">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.sites', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="siteslocation" title="Location">
                        {isSitesLocationActive && (  
                            <SitesLocation
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].location`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="sitesbgp" title="Bgp">
                        {isSitesBgpActive && (  
                            <SitesBgp
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="sitesospf" title="Ospf">
                        {isSitesOspfActive && (  
                            <SitesOspf
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="sitesospfpolicies" title="Ospf Policies">
                        {isSitesOspfPoliciesActive && (  
                            <SitesOspfPolicies
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].ospf_policies`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="sitespods" title="Pods">
                        {isSitesPodsActive && (  
                            <SitesPods
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].pods`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

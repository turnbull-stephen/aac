...
import PoliciesDhcpRelaysProviders from './PoliciesDhcpRelaysProviders'


const PoliciesDhcpRelays = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isPoliciesDhcpRelaysProvidersActive = activeKeys[item.id] === 'policiesdhcprelaysproviders';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'policiesdhcprelays'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="policiesdhcprelays" title="Dhcp Relays">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.policies_dhcp_relays', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="policiesdhcprelaysproviders" title="Providers">
                        {isPoliciesDhcpRelaysProvidersActive && (  
                            <PoliciesDhcpRelaysProviders
                              key={item.id}
                              control={control}
...
export default PoliciesDhcpRelays;

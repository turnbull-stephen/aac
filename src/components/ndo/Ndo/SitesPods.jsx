...
import SitesPodsSpines from './SitesPodsSpines'


const SitesPods = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isSitesPodsSpinesActive = activeKeys[item.id] === 'sitespodsspines';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'sitespods'}
                    ...
                    >
                      <Tab eventKey="sitespods" title="Pods">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'ndo.sites_pods', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isSitesPodsSpinesActive && (  
                            <SitesPodsSpines
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].spines`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

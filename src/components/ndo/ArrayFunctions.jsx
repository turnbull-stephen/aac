...
const ArrayFunctions = ({
  ...
  if (value.type === 'select-ref') {
    ...
    if (refParts.length === 2) {
      const refOptions = Array.isArray(formData[refParts[0]]) ? formData[refParts[0]].map((data) => ({ value: data[refParts[1]], label: data[refParts[1]] })) : [];
      options.push(refOptions)
    }
    if (refParts.length === 3) {
      const refOptions = Array.isArray(formData[refParts[0][1]]) ? formData[refParts[0][1]].map((data) => ({ value: data[refParts[2]], label: data[refParts[2]] })) : [];
      options.push(refOptions)
    }

    //const options = Array.isArray(formData[refParts[0]]) ? formData[refParts[0]].map((data) => ({ value: data[refParts[1]], label: data[refParts[1]] })) : [];
    return (

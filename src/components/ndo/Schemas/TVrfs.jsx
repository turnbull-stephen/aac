...
import TVrfSites from './TVrfSites'
import TVrfsContracts from './TVrfsContracts'


const TVrfs = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTVrfSitesActive = activeKeys[item.id] === 'tvrfsites';
          const isTVrfsContractsActive = activeKeys[item.id] === 'tvrfscontracts';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tvrfs'}
                    ...
                    >
                      <Tab eventKey="tvrfs" title="Vrfs">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_vrfs', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tvrfsites" title="Sites">
                        {isTVrfSitesActive && (  
                            <TVrfSites
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tvrfscontracts" title="Contracts">
                        {isTVrfsContractsActive && (  
                            <TVrfsContracts
                              key={item.id}
                              control={control}

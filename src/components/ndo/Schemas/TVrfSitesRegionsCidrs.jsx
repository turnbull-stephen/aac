...
import TVrfSitesRegionsCidrsSubnets from './TVrfSitesRegionsCidrsSubnets'


const TVrfSitesRegionsCidrs = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTVrfSitesRegionsCidrsSubnetsActive = activeKeys[item.id] === 'tvrfsitesregionscidrssubnets';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tvrfsitesregionscidrs'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tvrfsitesregionscidrs" title="Cidrs">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_vrf_sites_regions_cidrs', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tvrfsitesregionscidrssubnets" title="Subnets">
                        {isTVrfSitesRegionsCidrsSubnetsActive && (  
                            <TVrfSitesRegionsCidrsSubnets
                              key={item.id}
                              control={control}
...
export default TVrfSitesRegionsCidrs;

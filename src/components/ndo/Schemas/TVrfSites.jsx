...
import TVrfSitesRegions from './TVrfSitesRegions'


const TVrfSites = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTVrfSitesRegionsActive = activeKeys[item.id] === 'tvrfsitesregions';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tvrfsites'}
                    ...
                    >
                      <Tab eventKey="tvrfsites" title="Sites">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_vrf_sites', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        {isTVrfSitesRegionsActive && (  
                            <TVrfSitesRegions
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].regions`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

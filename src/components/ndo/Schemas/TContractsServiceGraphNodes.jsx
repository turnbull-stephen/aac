...
import TContractsServiceGraphNodesProv from './TContractsServiceGraphNodesProv'
import TContractsServiceGraphNodesCon from './TContractsServiceGraphNodesCon'


const TContractsServiceGraphNodes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTContractsServiceGraphNodesProvActive = activeKeys[item.id] === 'tcontractsservicegraphnodesprov';
          const isTContractsServiceGraphNodesConActive = activeKeys[item.id] === 'tcontractsservicegraphnodescon';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tcontractsservicegraphnodes'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tcontractsservicegraphnodes" title="Nodes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_contracts_service_graph_nodes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tcontractsservicegraphnodesprov" title="Provider">
                        {isTContractsServiceGraphNodesProvActive && (  
                            <TContractsServiceGraphNodesProv
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tcontractsservicegraphnodescon" title="Consumer">
                        {isTContractsServiceGraphNodesConActive && (  
                            <TContractsServiceGraphNodesCon
                              key={item.id}
                              control={control}
...
export default TContractsServiceGraphNodes;

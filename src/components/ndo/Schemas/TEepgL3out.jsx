...
const  TEepgL3out = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('teepgl3out'); // Keep state for the Tab selection
  ...
  const hasTEepgL3outErrors = Boolean(errors && errors[name] && errors[name].l3out)//;
  
  const isTEepgL3outActive = key === 'teepgl3out';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        
        
        <Tab eventKey="teepgl3out" title={tabTitle('L3out', hasTEepgL3outErrors)}>
          {isTEepgL3outActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'schemas.t_eepg_l3out', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

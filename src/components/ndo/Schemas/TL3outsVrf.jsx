...
const  TL3outsVrf = ({ control, register, errors, name, getValues, setValue, watch, setError, clearErrors }) => {
   
  const [key, setKey] = useState('tl3outsvrf'); // Keep state for the Tab selection
  ...
  const hasTL3outsVrfErrors = Boolean(errors && errors[name] && errors[name].vrf)//;
  
  const isTL3outsVrfActive = key === 'tl3outsvrf';
  return (
    <div>
      <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 tabs-title`} >
        
        
        <Tab eventKey="tl3outsvrf" title={tabTitle('Vrf', hasTL3outsVrfErrors)}>
          {isTL3outsVrfActive && (
            <Card>
              <Card.Body> 
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}> 
                  {Object.entries(get(schema, 'schemas.t_l3outs_vrf', {})).map(([objectKey, value]) => (
                    <ObjectFunctions
                      key={objectKey}
                      objectKey={objectKey}
                      value={value}

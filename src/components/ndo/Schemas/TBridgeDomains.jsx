...
import TBdVrf from './TBdVrf'
import TBdSubnets from './TBdSubnets'
import TBdSites from './TBdSites'
import TBdDhcpPolicies from './TBdDhcpPolicies'


const TBridgeDomains = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTBdVrfActive = activeKeys[item.id] === 'tbdvrf';
          const isTBdSubnetsActive = activeKeys[item.id] === 'tbdsubnets';
          const isTBdSitesActive = activeKeys[item.id] === 'tbdsites';
          const isTBdDhcpPoliciesActive = activeKeys[item.id] === 'tbddhcppolicies';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tbridgedomains'}
                    ...
                    >
                      <Tab eventKey="tbridgedomains" title="Bridge Domains">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_bridge_domains', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tbdsubnets" title="Subnets">
                        {isTBdSubnetsActive && (  
                            <TBdSubnets
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tbdsites" title="Sites">
                        {isTBdSitesActive && (  
                            <TBdSites
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tbddhcppolicies" title="Dhcp Policies">
                        {isTBdDhcpPoliciesActive && (  
                            <TBdDhcpPolicies
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].dhcp_policies`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

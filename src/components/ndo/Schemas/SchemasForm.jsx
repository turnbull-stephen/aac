...
import SchemasTemplates from './SchemasTemplates';
...
const SchemasForm = () => {
      ...
      if (fileExtension === 'yaml' || fileExtension === 'yml') {
        ...
        reader.onload = (event) => {
          try {
            ...
            reset(loadedData.ndo.schemas[0]);
            onSubmit(loadedData.ndo.schemas[0])
  ...
  // Define the Root for ndo/schemas
  ...
  const onSubmit = (data) => {
    ...
    root.ndo.schemas.push(processedData);
    
    // To display the YAML representation of the schemas array,
  ...
  const [key, setKey] = useState('schemas'); // Keep state for the Tab selection
  


  const hasSchemasErrors =Boolean(errors.name);
  const hasSchemasTemplatesErrors = Boolean(errors.templates);
                  ...
                  <Card>
                    <Card.Body>
                      <Tabs
                        ...
                        //fill
                      >
                        <Tab eventKey="schemas" title={tabTitle('Schemas', hasSchemasErrors)}>
                          <Card>
                            <Card.Body>
                             
                            <Schemas control={control} register={register} errors={errors} name={''} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        <Tab eventKey="schemastemplates" title={tabTitle('Templates', hasSchemasTemplatesErrors)}>
                          <Card>
                            <Card.Body>
                            <SchemasTemplates control={control} register={register} errors={errors} clearErrors={clearErrors} setError={setError} name={'templates'} getValues={getValues} setValue={setValue} watch={watch} />
                            </Card.Body>
                          </Card>
                        </Tab>
                        </Tabs>

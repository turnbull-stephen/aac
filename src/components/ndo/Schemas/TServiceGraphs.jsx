...
import TServiceGraphsNodes from './TServiceGraphsNodes'


const TServiceGraphs = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTServiceGraphsNodesActive = activeKeys[item.id] === 'tservicegraphsnodes';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tservicegraphs'}
                        ...
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_service_graphs', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 

...
import TApEpgSitesSelectorsExpressions from './TApEpgSitesSelectorsExpressions'


const TApEpgSitesSelectors = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTApEpgSitesSelectorsExpressionsActive = activeKeys[item.id] === 'tapepgsitesselectorsexpressions';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tapepgsitesselectors'}
                    ...
                    >
                      <Tab eventKey="tapepgsitesselectors" title="Selectors">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_ap_epg_sites_selectors', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tapepgsitesselectorsexpressions" title="Expressions">
                        {isTApEpgSitesSelectorsExpressionsActive && (  
                            <TApEpgSitesSelectorsExpressions
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].expressions`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
...
export default TApEpgSitesSelectors;

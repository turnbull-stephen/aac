...
import TContractsFilters from './TContractsFilters'
import TContractsServiceGraph from './TContractsServiceGraph'


const TContracts = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTContractsFiltersActive = activeKeys[item.id] === 'tcontractsfilters';
          const isTContractsServiceGraphActive = activeKeys[item.id] === 'tcontractsservicegraph';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tcontracts'}
                    ...
                    >
                      <Tab eventKey="tcontracts" title="Contracts">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_contracts', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tcontractsfilters" title="Filters">
                        {isTContractsFiltersActive && (  
                            <TContractsFilters
                              key={item.id}
                              control={control}
                        ...
                        <Tab eventKey="tcontractsfilters" title="Provider To Consumer Filters">
                        {isTContractsFiltersActive && (  
                            <TContractsFilters
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].provider_to_consumer_filters`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tcontractsfilters" title="Consumer To Provider Filters">
                        {isTContractsFiltersActive && (  
                            <TContractsFilters
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].consumer_to_provider_filters`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}
                        ...
                        <Tab eventKey="tcontractsservicegraph" title="Service Graph">
                        {isTContractsServiceGraphActive && (  
                            <TContractsServiceGraph
                              key={item.id}
                              control={control}
                              name={`${name}[${originalIndex}].service_graph`}
                              register={register}
                              errors={errors}
                              setError={setError}
                              clearErrors={clearErrors}

...
import TServiceGraphsNodesSites from './TServiceGraphsNodesSites'


const TServiceGraphsNodes = React.memo(({ control, register, errors, clearErrors, setError, name }) => {
    ...
    <>
      {fields.length === 0 ? (
        ...
        paginatedFields.map((item, index) => {
          ...
          const isTServiceGraphsNodesSitesActive = activeKeys[item.id] === 'tservicegraphsnodessites';
            ...
            <div key={item.id}>
              <Accordion className='mb-2'>
                <Accordion.Item eventKey={originalIndex.toString()}>
                  ...
                  <Accordion.Body>
                    <Tabs
                      activeKey={activeKeys[item.id] || 'tservicegraphsnodes'}
                      onSelect={(k) => handleTabChange(item.id, k)}
                      className="mb-2"
                      id={item.id}
                    >
                      <Tab eventKey="tservicegraphsnodes" title="Nodes">
                        <Card>
                          <Card.Body>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gridGap: '1rem' }}>
                              {Object.entries(get(schema, 'schemas.t_service_graphs_nodes', {})).map(([objectKey, value]) => (
                                <ArrayFunctions
                                  key={objectKey} 
                        ...
                        <Tab eventKey="tservicegraphsnodessites" title="Sites">
                        {isTServiceGraphsNodesSitesActive && (  
                            <TServiceGraphsNodesSites
                              key={item.id}
                              control={control}
...
export default TServiceGraphsNodes;
